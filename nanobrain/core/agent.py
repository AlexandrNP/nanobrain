...
class AgentConfig(ConfigBase):
    ...
    **Advanced Configuration Patterns:**
    ...
    prompt_template_file: Optional[str] = Field(
        default=None, description="Path to YAML file containing prompt templates")
    executor_config: Optional[ExecutorConfig] = None
    tools: List[Dict[str, Any]] = Field(default_factory=list)
    tools_config_path: Optional[str] = Field(
        default=None, description="Path to YAML file containing tool configurations")
    ...
    # Specialized agent configuration fields
    confidence_threshold: Optional[float] = Field(
        default=0.7, ge=0.0, le=1.0, description="Confidence threshold for specialized agents")
    analysis_depth: Optional[str] = Field(
        default="standard", description="Analysis depth for specialized agents")
    biological_context_focus: Optional[bool] = Field(
        default=True, description="Enable biological context focus for specialized agents")

    # MANDATORY: Agent card section for A2A protocol compliance
    agent_card: Optional[Dict[str, Any]] = Field(
        default=None, description="Agent card metadata for A2A protocol compliance")
    ...
    def from_yaml(cls, yaml_path: str) -> 'AgentConfig':
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"Agent config file not found: {yaml_path}")
        ...
        try:
            ...
            logger.error(
                f"Error creating AgentConfig from {yaml_path}: {e}", exc_info=True)
            raise ValueError(
                f"Error creating AgentConfig from {yaml_path}: {e}")
    ...
    **Advanced Features:**
    ...
    def _init_from_config(self, config: Any, component_config: Dict[str, Any],
        ...
        if executor:
            ...
            # Try multiple paths to find the default executor config
            default_executor_paths = [
                'config/core/executor/default_executor.yml',  # Original path
                'Chatbot/config/core/executor/default_executor.yml',  # Chatbot-relative path
                '../config/core/executor/default_executor.yml',  # Server-relative path
                '../../config/core/executor/default_executor.yml'  # Deep server-relative path
            ]

            executor_config = getattr(config, 'executor_config', None)
            if not executor_config:
                for path in default_executor_paths:
                    try:
                        executor_config = ExecutorConfig.from_config(path)
                        break
                    except Exception:
                        continue

                # If no config found, create minimal default using framework pattern
                if not executor_config:
                    # Create a temporary default config file if none exists
                    import tempfile
                    import yaml
                    import os

                    default_config = {
                        'name': 'temp_default_executor',
                        'description': 'Temporary default executor',
                        'class': 'nanobrain.core.executor.LocalExecutor',
                        'config': {
                            'executor_type': 'local',
                            'max_workers': 2,
                    ...
                    try:
                        executor_config = ExecutorConfig.from_config(temp_config_path)
                    finally:
    ...
    def _init_prompt_manager(self) -> None:
        ...
        try:
            ...
            if self.config.prompt_template_file:
                ...
                if not Path(template_path).is_absolute():
                    ...
                    possible_paths = [
                        ...
                        # Relative to nanobrain package
                        Path(__file__).parent.parent / template_path,
                        Path(__file__).parent.parent.parent /
                        template_path,  # Relative to project root
                    ]
                    ...
                    for path in possible_paths:
                        ...
                        self.agent_logger.warning(
                            f"Prompt template file not found in expected locations: {self.config.prompt_template_file}")
            ...
            self.agent_logger.warning(
                f"Failed to initialize prompt manager: {e}", exc_info=True)
    ...
    async def initialize(self) -> None:
        ...
        if self._is_initialized:
            self.agent_logger.log_debug(
                f"Agent {self.name} already initialized")
            return

        # Initialize executor
        self.agent_logger.log_debug(
            f"Initializing executor for agent {self.name}")
        await self.executor.initialize()

        # Initialize LLM client
        self.agent_logger.log_debug(
            f"Initializing LLM client for agent {self.name}")
        await self._initialize_llm_client()

        # Load tools from YAML configuration if specified
        if self.config.tools_config_path:
            self.agent_logger.log_debug(
                f"Loading tools from YAML config: {self.config.tools_config_path}")
            await self._load_tools_from_yaml_config()

        # Register tools from configuration
        self.agent_logger.log_debug(
            f"Registering {len(self.config.tools)} tools for agent {self.name}")
        for i, tool_config in enumerate(self.config.tools):
            try:
                await self._register_tool_from_config(tool_config)
                self.agent_logger.log_debug(
                    f"Registered tool {i+1}/{len(self.config.tools)}")
            except Exception as e:
        ...
        self.agent_logger.log_debug(
            f"Initializing all tools for agent {self.name}")
    ...
    async def _initialize_llm_client(self) -> None:
        ...
        try:
            ...
            safe_log(
                'error', f"Failed to initialize LLM client for agent {getattr(self, 'name', 'unknown')}: {e}")
            self.llm_client = None
    ...
    async def _load_tools_from_yaml_config(self) -> None:
        ...
        try:
            ...
            config_path = self._resolve_config_path(
                self.config.tools_config_path)

            self.agent_logger.log_debug(
                f"Loading tools from YAML file: {config_path}")
            ...
            self.agent_logger.log_debug(
                f"Found {len(yaml_tools)} tools in YAML config")
            ...
            for tool_config in yaml_tools:
                ...
                standardized_config = self._standardize_tool_config(
                    tool_config)
    ...
    def _resolve_config_path(self, config_path: str) -> str:
        ...
        raise FileNotFoundError(
            f"Config file not found in any search paths: {config_path}")
    ...
    def _standardize_tool_config(self, tool_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tool_type = tool_config.get('tool_type', 'agent')
    ...
    async def _register_tool_from_config(self, tool_config: Dict[str, Any]) -> None:
        ...
        self.agent_logger.log_debug(
            f"Registering tool from config: {tool_config.get('name', 'unnamed')}")
        ...
        if tool_type == ToolType.FUNCTION:
            ...
            self.agent_logger.log_debug(
                f"Successfully registered tool: {config.name}")
    ...
    async def _register_agent_tool_from_config(self, tool_config: Dict[str, Any], config: ToolConfig) -> None:
        ...
        try:
            ...
            self.agent_logger.log_debug(
                f"Successfully registered agent tool: {config.name} ({class_path})")
    ...
    def _import_class_from_path(self, class_path: str):
        ...
        try:
            ...
            logger = get_logger("agent.import")
            logger.error(
                f"Error importing class {class_path}: {e}", exc_info=True)
    ...
    def register_tool(self, tool: ToolBase) -> None:
        ...
        self.tool_registry.register(tool)
        self.agent_logger.log_debug(
            f"Agent {self.name} registered tool: {tool.name}")
    ...
    async def aprocess(self, input_text: str, **kwargs) -> str:
        """
        Async process method - alias for process() for compatibility.

        This method provides the standard aprocess interface expected by
        workflows and other components while delegating to the process method.
        ...
        async with self.agent_logger.interaction_context(input_text) as context:
            ...
            try:
                ...
                if context:
                    ...
                    context['llm_calls'] = self._total_llm_calls - \
                        initial_llm_calls
                    context['total_tokens'] = self._total_tokens_used - \
                ...
                self.agent_logger.log_error(f"Agent {self.name} execution failed: {e}",
                                            error_type=type(e).__name__,
                                            context={
                                                "error_count": self._error_count, "input_length": len(input_text)},
                                            exc_info=True)
                raise
    ...
    async def _call_llm(self, messages: List[Dict[str, Any]], tools: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        ...
        api_call_start = start_time  # Initialize to avoid undefined variable errors

        try:
            # PHASE 1 DIAGNOSTICS: Enhanced LLM call tracing
            self.agent_logger.log_info(f"🔍 [LLM-TRACE] _call_llm() ENTRY - Agent: {getattr(self, 'name', 'unknown')}")
            self.agent_logger.log_info(f"🔍 [LLM-TRACE] Messages count: {len(messages)}")
            self.agent_logger.log_info(f"🔍 [LLM-TRACE] Tools provided: {tools is not None}")
            self.agent_logger.log_info(f"🔍 [LLM-TRACE] LLM client type: {type(self.llm_client)}")
            ...
            self.agent_logger.log_info(f"🔍 [LLM-TRACE] Request params prepared - model: {self.config.model}")
            ...
            if hasattr(response, 'usage') and response.usage:
                ...
                self.agent_logger.log_info(f"🔍 [LLM-TRACE] Tokens used: {tokens_used}")
            ...
            # PHASE 1 DIAGNOSTICS: Final response tracing
            total_duration = time.time() - start_time
            self.agent_logger.log_info(f"✅ [LLM-TRACE] _call_llm() completed successfully in {total_duration:.2f}s")
            self.agent_logger.log_info(f"🔍 [LLM-TRACE] Response content length: {len(message_content)}")

            return response_dict

        except Exception as e:
            # PHASE 1 DIAGNOSTICS: Enhanced exception tracing with safe variable access
            current_time = time.time()
            api_call_duration = current_time - api_call_start if 'api_call_start' in locals() else 0
            total_duration = current_time - start_time if 'start_time' in locals() else 0

            self.agent_logger.log_error(f"❌ [LLM-TRACE] LLM call failed after {total_duration:.2f}s (API: {api_call_duration:.2f}s): {e}",
                                        error_type=type(e).__name__,
                                        exc_info=True)
            self.agent_logger.log_error(f"❌ [LLM-TRACE] Exception type: {type(e)}")
            self.agent_logger.log_error(f"❌ [LLM-TRACE] Exception args: {e.args}")
            raise
    ...
    async def _execute_tool_calls(self, tool_calls: List[Any]) -> List[Dict[str, Any]]:
        ...
        for tool_call in tool_calls:
            ...
            try:
                ...
                self.agent_logger.log_debug(
                    f"Tool call completed: {tool_name} in {duration_ms:.1f}ms")
    ...
    def _setup_agent_executor(self):
        ...
        if not self.tool_registry.list_tools():
            self.agent_logger.log_debug(
                "No tools available, skipping agent executor setup")
            return
        ...
        try:
            ...
            for tool_name in self.tool_registry.list_tools():
                tool = self.tool_registry.get(tool_name)
                if tool:
                    langchain_tool = tool.to_langchain_tool() if hasattr(
                        tool, 'to_langchain_tool') else None
                    if langchain_tool:
            ...
            self.agent_logger.log_debug(
                f"Agent executor set up with {len(langchain_tools)} tools")
    ...
    def add_to_conversation(self, role: str, content: str) -> None:
        ...
        self.agent_logger.log_debug(
            f"Added message to conversation: {role} ({len(content)} chars)")
    ...
    def clear_conversation(self) -> None:
        ...
        self.agent_logger.log_debug(
            f"Cleared conversation history ({old_length} messages)")
...
class ConversationalAgent(Agent):
    ...
    async def process(self, input_text: str, **kwargs) -> str:
        ...
        # Debug logging for ConversationalAgent.process()
        self.agent_logger.log_info(
            f"🔍 ConversationalAgent.process() called with: {input_text[:50]}...")
        self.agent_logger.log_info(
            f"🔍 LLM client available: {self.llm_client is not None}")
        ...
        async with self.agent_logger.interaction_context(input_text) as context:
            if not self.llm_client:
                ...
                history_start = max(
                    0, len(self._conversation_history) - self.max_history_length)
                ...
                try:
                    self.agent_logger.log_info(
                        f"🔍 Calling _call_llm() with {len(messages)} messages...")
                    llm_response = await self._call_llm(messages, tools)
                    self.agent_logger.log_info(
                        f"🔍 _call_llm() returned response: {str(llm_response)[:100]}...")
                except Exception as e:
...
def create_langchain_agent_executor(agents: List[Agent], llm_model: str = "gpt-3.5-turbo"):
    ...
    if not LANGCHAIN_AVAILABLE:
        raise ImportError(
            "LangChain is not available. Install with: pip install langchain langchain-openai")
    ...
    try:
        ...
        for agent in agents:
            try:
                ...
                logger.warning(
                    f"Failed to convert agent {agent.name} to LangChain tool: {e}", exc_info=True)
