...
class AgentLogger:
    ...
    def __init__(self, agent_name: str, agent_type: str):
        ...
        self._is_concrete_instance = self._is_concrete_agent_instance(
            agent_type)
        ...
        if self._is_concrete_instance:
            try:
                self.logger = get_process_safe_logger(
                    agent_name, category="agents")
                ...
                print(
                    f"Failed to initialize agent logger for {agent_name}: {e}")
                self.logger = None
        else:
    ...
    def _is_concrete_agent_instance(self, agent_type: str) -> bool:
        ...
        is_specific_agent = (
            ...
            (agent_type.endswith('Agent') and agent_type !=
             'Agent' and len(agent_type) > 5)
        )
    ...
    def info(self, message: str, **metadata):
        """Compatibility method for direct info logging."""
        return self.log_info(message, **metadata)
    ...
    async def interaction_context(self, input_text: str):
        ...
        try:
            ...
            response_text = context.get(
                'response_text', '') if 'context' in locals() else ''
            raise
...
def create_agent_logger(agent_instance: Any) -> AgentLogger:
    ...
    if hasattr(agent_instance, 'config'):
        enable_detailed = getattr(
            agent_instance.config, 'enable_logging', True)
