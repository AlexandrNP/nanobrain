...
class ExternalToolConfig(ConfigBase):
    ...
    progressive_scaling: Dict[int, Dict[str, Any]
...
class ExternalTool(FromConfigBase, ABC):
        ...
        try:
            if getattr(self.config, 'verify_on_init', True):
                if not await self.verify_installation():
                    self.logger.warning(
                        f"Tool {self.tool_name} verification failed")
                    if await self.install_if_missing():
                        self.logger.info(
                            f"Tool {self.tool_name} installed successfully")
                    else:
                        raise ToolInstallationError(
                            f"Failed to install {self.tool_name}")
                else:
                    self.logger.info(
                        f"Tool {self.tool_name} verification successful")
        except Exception as e:
            self.logger.error(
                f"Tool initialization failed: {e}", exc_info=True)
            raise
    ...
    async def verify_installation(self) -> bool:
        ...
        if status.found:
            self.logger.info(
                f"✅ {self.tool_name} found: {status.installation_type}")
            return True
        ...
        if any([
            ...
            self.logger.info(
                f"🔄 {self.tool_name} not found, attempting installation...")
            ...
            if installation_success:
                ...
                if status.found:
                    self.logger.info(
                        f"✅ {self.tool_name} installed and configured successfully")
                    return True
    ...
    async def detect_existing_installation(self) -> InstallationStatus:
        ...
        try:
            ...
            self.logger.error(
                f"Installation detection failed: {e}", exc_info=True)
    ...
    async def _check_local_installation(self, local_path: str, status: InstallationStatus) -> bool:
        ...
        try:
            ...
            status.issues.append(
                f"Error checking local path {local_path}: {e}")
            self.logger.error(
                f"Error checking local path {local_path}: {e}", exc_info=True)
        return False
    ...
    async def _check_conda_environments(self, status: InstallationStatus) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Error checking conda environments: {e}", exc_info=True)
    ...
    async def _check_system_path(self, status: InstallationStatus) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Error checking system PATH: {e}", exc_info=True)
            ...
            try:
                if await self._check_tool_in_directory(path):
                    status.suggestions.append(
                        f"Tool might be available in: {path}")
            except Exception:
                continue
    ...
    async def install_if_missing(self) -> bool:
        ...
        self.logger.warning(
            f"Auto-installation not implemented for {self.tool_name}")
        return False
    ...
    async def auto_install(self) -> bool:
        ...
        self.logger.error(
            f"❌ All auto-installation methods failed for {self.tool_name}")
        return False
    ...
    async def _install_via_conda(self) -> bool:
        ...
        try:
            conda_package = getattr(self.config, 'conda_package', None)
            conda_channel = getattr(
                self.config, 'conda_channel', 'conda-forge')
            environment_name = getattr(self.config, 'environment_name', None)
            create_isolated = getattr(
                self.config, 'create_isolated_environment', False)

            if not conda_package:
                return False

            self.logger.info(
                f"📦 Installing {conda_package} via conda from {conda_channel}")
            ...
            if create_isolated and environment_name:
                ...
                cmd = ["conda", "install", "-n", environment_name,
                       "-c", conda_channel, conda_package, "-y"]
            else:
                # Install in current environment
                cmd = ["conda", "install", "-c",
                       conda_channel, conda_package, "-y"]

            result = await self._execute_single_command(cmd)
            if result.success:
                self.logger.info(
                    f"✅ Successfully installed {conda_package} via conda")
                return True
            else:
                self.logger.error(
                    f"❌ Conda installation failed: {result.stderr_text}")
                return False

        except Exception as e:
            self.logger.error(
                f"❌ Error during conda installation: {e}", exc_info=True)
            return False
    ...
    async def _install_via_pip(self) -> bool:
        ...
        try:
            ...
            if result.success:
                self.logger.info(
                    f"✅ Successfully installed {pip_package} via pip")
                return True
            else:
                self.logger.error(
                    f"❌ Pip installation failed: {result.stderr_text}")
                return False

        except Exception as e:
            self.logger.error(
                f"❌ Error during pip installation: {e}", exc_info=True)
            return False
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if not result.success:
                    self.logger.error(
                        f"❌ Git clone failed: {result.stderr_text}")
                    return False
            ...
            self.logger.error(
                f"❌ Error during git installation: {e}", exc_info=True)
            return False
    ...
    async def _create_conda_environment(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"🔧 Creating conda environment: {environment_name}")
            ...
            if result.success:
                self.logger.info(
                    f"✅ Created conda environment: {environment_name}")
                return True
            else:
                self.logger.error(
                    f"❌ Failed to create environment: {result.stderr_text}")
                return False

        except Exception as e:
            self.logger.error(
                f"❌ Error creating conda environment: {e}", exc_info=True)
            return False
    ...
    async def _build_tool_standard(self, source_dir: str) -> bool:
        ...
        try:
            ...
            if (source_path / "setup.py").exists():
                ...
                cmd = ["cmake", "--build",
                       str(build_dir), "--target", "install"]
            else:
                self.logger.warning(
                    f"⚠️ No recognized build system found in {source_dir}")
                return False
            ...
            self.logger.error(
                f"❌ Error during standard build: {e}", exc_info=True)
            return False
    ...
    async def _execute_with_retry(self, command: List[str], **kwargs) -> ToolResult:
        ...
        for attempt in range(self.retry_attempts):
            try:
                self.logger.debug(
                    f"Executing {self.tool_name} command (attempt {attempt + 1}): {' '.join(command)}")
                ...
                if result.success:
                    ...
                    last_error = ToolExecutionError(
                        f"Command failed: {result.stderr_text}")
    ...
    async def _execute_single_command(self, command: List[str], **kwargs) -> ToolResult:
        ...
        try:
            ...
            self.logger.error(
                f"Command timeout after {self.timeout_seconds} seconds")
    ...
    async def cleanup_temp_file(self, filepath: str) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to cleanup temp file {filepath}: {e}", exc_info=True)
