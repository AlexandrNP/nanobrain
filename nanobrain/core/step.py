    ...
    **Advanced Features:**
    ...
    async def _execute_on_trigger(self, trigger_event: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # ✅ CRITICAL DEBUG: Add logging before calling process
            self.nb_logger.info(f"🚀 ABOUT TO CALL PROCESS METHOD for step {self.name}")
            self.nb_logger.info(f"🚀 Input data keys: {list(input_data.keys())}")

            # Execute step business logic
            result = await self.process(input_data)

            # ✅ CRITICAL DEBUG: Add logging after calling process
            self.nb_logger.info(f"🚀 PROCESS METHOD COMPLETED for step {self.name}")
    ...
    async def _process_with_agent(self, agent_input: str, conversation_history: List[Dict[str, Any]],
        ...
        try:
            ...
            if conversation_history and hasattr(self.wrapped_agent, 'process_with_history'):
                ...
                result = await self.wrapped_agent.process_with_history(agent_input, conversation_history)
            elif hasattr(self.wrapped_agent, 'process'):
                # Standard agent process method
                result = await self.wrapped_agent.process(agent_input)
            elif hasattr(self.wrapped_agent, 'execute'):
                # Alternative execute method
                result = await self.wrapped_agent.execute(agent_input)
            elif hasattr(self.wrapped_agent, 'chat'):
                # Chat-specific method
                result = await self.wrapped_agent.chat(agent_input)
            else:
                raise RuntimeError(
