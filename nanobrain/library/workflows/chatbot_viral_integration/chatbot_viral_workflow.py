...
class ChatbotViralWorkflow(Workflow):
    ...
    def _init_from_config(self, config: WorkflowConfig, component_config: Dict[str, Any],
        ...
        self.yaml_config = config.__dict__ if hasattr(
        ...
        self.nb_logger.info(
            "🤖 Chatbot Viral Integration Workflow initialized with AlphavirusWorkflow as step")
    ...
    def _setup_workflow_triggers(self) -> None:
        ...
        try:
            ...
            self.nb_logger.info(
                "✅ Triggers will be loaded from configuration files automatically")
            ...
            self.nb_logger.info(
                "🔥 Workflow-level triggers registered for event-driven execution")

            return True
        except Exception as e:
            self.nb_logger.error(
                f"❌ Failed to setup workflow triggers: {e}", exc_info=True)
    ...
    async def _execute_workflow_on_trigger(self, trigger_event: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.nb_logger.info(
                f"🚀 Workflow triggered by data unit change: {input_data}")
            ...
            if result:
                ...
                self.nb_logger.info(
                    "✅ Workflow execution completed and result set")
            else:
                self.nb_logger.warning(
                    "⚠️ Workflow execution returned no result")
    ...
    async def process_user_message(self, user_message: str, session_id: Optional[str] = None) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            ...
            self.nb_logger.info(
                f"🔄 Processing message with data-driven architecture for session {session_id}: '{user_message[:100]}...'")
            ...
            if result and 'formatted_response' in result:
                assistant_msg.content = result['formatted_response'].get(
                    'content', '')
            ...
            self.nb_logger.error(
                f"❌ Error processing user message: {e}", exc_info=True)
    ...
    async def process_with_data_driven_architecture(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.nb_logger.info(
                "✅ Data-driven workflow execution completed successfully")
            return final_result

        except Exception as e:
            self.nb_logger.error(
                f"❌ Data-driven workflow execution failed: {e}", exc_info=True)

            # Log full traceback for debugging
            import traceback
            full_traceback = traceback.format_exc()
            self.nb_logger.error(
                f"Workflow execution traceback: {full_traceback}")
    ...
    async def _trigger_workflow_execution(self, input_data: Dict[str, Any]) -> None:
        ...
        self.nb_logger.info(
            "🚀 Workflow execution triggered via data unit update")
    ...
    async def _wait_for_workflow_completion(self, timeout: int = 300) -> Dict[str, Any]:
        ...
        while time.time() - start_time < timeout:
            ...
            if final_result is not None:
                self.nb_logger.info(
                    "✅ Workflow completed - final result available")
                return final_result
    ...
    async def _create_step_instance(self, step_id: str, step_config: StepConfig, config_dict: Dict[str, Any]) -> Step:
        ...
        try:
            ...
            self.workflow_logger.info(
                f"✅ Created step instance: {step_id} ({step_class})")
            return step_instance
        except Exception as e:
            self.workflow_logger.error(
                f"❌ Failed to create step {step_id}: {e}", exc_info=True)
            import traceback
            self.workflow_logger.error(
                f"Step creation traceback: {traceback.format_exc()}")
            raise
