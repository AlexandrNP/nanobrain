...
Return only the clean virus name, nothing else:"""
    ...
    async def execute(self, **kwargs) -> Dict[str, Any]:
        """
        Execute the BV-BRC data acquisition step using NanoBrain framework pattern
        
        Args:
            **kwargs: Keyword arguments from framework execution
            
        Returns:
            Dict containing all acquired and processed data
        """
        
        # Extract input_data from kwargs (framework passes this)
        input_data = kwargs.get('input_data', {})
        
        # Handle both direct input_data and legacy input_params pattern
        if 'input_params' in kwargs:
            input_params = kwargs['input_params']
        else:
            # Convert input_data to input_params format
            input_params = input_data if isinstance(input_data, dict) else {}
        
        # Call the original execute logic with proper parameter format
        return await self._execute_data_acquisition(input_params)
    
    async def _execute_data_acquisition(self, input_params: Dict[str, Any]) -> Dict[str, Any]:
        """
