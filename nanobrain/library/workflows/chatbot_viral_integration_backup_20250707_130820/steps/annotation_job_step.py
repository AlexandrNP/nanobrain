...
class AnnotationJobStep(Step):
    ...
    async def _extract_analysis_parameters(self, job_data: AnnotationJobData) -> Dict[str, Any]:
        ...
        # Pass organism information to data acquisition step for CSV search
        if 'organisms' in extracted_params and extracted_params['organisms']:
            ...
            # 🚨 CRITICAL: CSV search is now done in data acquisition step
            # Annotation job step only passes virus species to data acquisition
            self.nb_logger.info(f"🔍 Passing virus species '{target_organism}' to data acquisition step for CSV search")
