...
class ResponseFormattingStep(Step):
    ...
    async def process(self, input_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        Args:
            ...
            **kwargs: Additional keyword arguments from workflow execution
    ...
    async def _format_annotation_job_response(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # DEBUG: Log job status and result for clustering investigation
        self.nb_logger.info(f"🔍 DEBUG: Response formatter - job status: {job_data.status}")
        if job_data.result:
            result_keys = list(job_data.result.keys()) if isinstance(job_data.result, dict) else 'not a dict'
            self.nb_logger.info(f"🔍 DEBUG: Response formatter - result keys: {result_keys}")
            if isinstance(job_data.result, dict) and 'protein_clusters' in job_data.result:
    ...
    async def _format_clustering_analysis_response(self, job_data: AnnotationJobData, 
                                                 results: Dict[str, Any]) -> Dict[str, Any]:
        """
        Format clustering analysis results with detailed markdown display
        
        Args:
            job_data: Contains job_id and processing_time
            results: Contains protein_clusters, clustering_analysis, and other analysis results
            
        Returns:
            Formatted response with presentation metadata for clustering results
        """
        try:
            protein_clusters = results.get('protein_clusters', {})
            clustering_analysis = results.get('clustering_analysis', {})
            clustering_method = results.get('clustering_method', 'unknown')
            
            # Debug logging for cluster data
            if protein_clusters:
                sample_cluster_id = next(iter(protein_clusters.keys()))
                sample_cluster = protein_clusters[sample_cluster_id]
                self.nb_logger.info(f"🔍 DEBUG: Response formatter - sample cluster '{sample_cluster_id}' keys: {list(sample_cluster.keys())}")
                self.nb_logger.info(f"🔍 DEBUG: Response formatter - length_stats: {sample_cluster.get('length_stats', 'MISSING')}")
                self.nb_logger.info(f"🔍 DEBUG: Response formatter - representative_sequence length: {len(sample_cluster.get('representative_sequence', ''))}")
                self.nb_logger.info(f"🔍 DEBUG: Response formatter - genome_distribution: {sample_cluster.get('genome_distribution', 'MISSING')}")
            
            # Handle None execution_time
            execution_time = job_data.execution_time or 0.0
            
            content_parts = [
                "✅ **Viral Protein Clustering Analysis Completed**",
                "",
                f"**Job ID**: `{job_data.job_id}`",
            ...
            # Add clustering overview
            total_clusters = len(protein_clusters)
            total_proteins = sum(cluster.get('cluster_size', 0) for cluster in protein_clusters.values())
            ...
            content_parts.extend([
                ...
                f"**Total Proteins Clustered**: {total_proteins:,}",
                ""
            ])
            
            # Add clustering statistics if available
            if clustering_analysis:
                stats = clustering_analysis.get('cluster_size_distribution', {})
                if stats:
                    content_parts.extend([
                        "## Cluster Statistics",
                        "",
                        f"**Average Cluster Size**: {stats.get('mean', 0):.1f}",
                        f"**Size Range**: {stats.get('min', 0)} - {stats.get('max', 0)} proteins",
                        f"**Standard Deviation**: {stats.get('std', 0):.1f}",
                        ""
                    ])
            
            # Add top clusters (largest ones)
            if protein_clusters:
                # Sort clusters by size (descending)
                sorted_clusters = sorted(
                    protein_clusters.items(), 
                    key=lambda x: x[1].get('cluster_size', 0), 
                    reverse=True
                )
                
                content_parts.extend([
                    "## Top Protein Clusters",
                    ""
                ])
                
                # Show top 50 clusters (or all if fewer than 50)
                display_count = min(50, len(sorted_clusters))
                for i, (cluster_id, cluster_data) in enumerate(sorted_clusters[:display_count]):
                    product_name = cluster_data.get('product_name', 'Unknown protein')
                    ...
                    content_parts.append(f"**{i+1}.** `{product_name}` ({cluster_size:,} proteins)")
                    
                    # Add detailed cluster information
                    if length_stats:
                        avg_length = length_stats.get('mean', 0)
                        min_length = length_stats.get('min', 0)
                        max_length = length_stats.get('max', 0)
                        std_length = length_stats.get('std', 0)
                        if avg_length > 0:
                            content_parts.append(f"   - Length: {avg_length:.0f} ± {std_length:.0f} aa (range: {min_length}-{max_length})")
                    
                    # Add genome distribution if available
                    genome_distribution = cluster_data.get('genome_distribution', {})
                    if genome_distribution:
                        unique_genomes = len(genome_distribution)
                        content_parts.append(f"   - Genomes: Found in {unique_genomes:,} different genomes")
                    
                    # Add representative sequence info if available
                    representative_seq = cluster_data.get('representative_sequence', '')
                    if representative_seq:
                        seq_preview = representative_seq[:50] + "..." if len(representative_seq) > 50 else representative_seq
                        content_parts.append(f"   - Representative: `{seq_preview}`")
                    
                    content_parts.append("")
                
                if len(sorted_clusters) > display_count:
                    remaining = len(sorted_clusters) - display_count
                    content_parts.extend([
                        f"*... and {remaining:,} more clusters*",
                        ""
                    ])
            
            # Add LLM summary if available
            llm_summary = clustering_analysis.get('llm_summary', '')
            if llm_summary:
                content_parts.extend([
                    "## Analysis Insights",
                    "",
            ...
            job_metadata = results.get('job_metadata', {})
            organism = job_metadata.get('organism', 'Unknown')
            analysis_type = job_metadata.get('analysis_type', 'viral_protein_clustering')
            ...
            content_parts.extend([
                ...
                f"**Clustering Algorithm**: {clustering_method}",
                ""
            ])
            
            # Add COMPLETE clustering data in JSON format - NO TRUNCATION
            if protein_clusters:
                # Include ALL clustering data - complete dataset
                complete_cluster_data = {}
                for cluster_id, cluster_data in protein_clusters.items():
                    complete_cluster_data[cluster_id] = {
                        'product_name': cluster_data.get('product_name', 'Unknown'),
                        'cluster_size': cluster_data.get('cluster_size', 0),
                        'length_stats': cluster_data.get('length_stats', {}),
                        'representative_sequence': cluster_data.get('representative_sequence', ''),
                        'genome_distribution': cluster_data.get('genome_distribution', {})
                    }
                
                content_parts.extend([
                    "## Complete Clustering Data",
                    "",
                    f"**Total Clusters**: {len(complete_cluster_data):,}",
                    "",
                    "```json",
                    json.dumps(complete_cluster_data, indent=2, ensure_ascii=False),
                    "```",
                    ""
                ])
            ...
            return {
                ...
                'clustering_data': {
                    ...
                    'all_clusters': protein_clusters,  # Include ALL clusters, not just top 10
                    'sorted_clusters': sorted(protein_clusters.items(), key=lambda x: x[1].get('cluster_size', 0), reverse=True) if protein_clusters else []
                },
                'analysis_metadata': {
                    'organism': organism,
            ...
            self.nb_logger.error(f"Error formatting clustering response: {e}")
            return {
                'content': f"✅ **Clustering Analysis Completed with Formatting Issues**\n\n**Job ID**: `{job_data.job_id}`\n\nThe clustering analysis completed successfully, but there was an issue formatting the results for display: {str(e)}",
                'message_type': MessageType.ANNOTATION_RESULT.value,
                'requires_markdown': True,
                'job_id': job_data.job_id,
                'status': "completed_with_errors",
...
- Check your input format"""
    ...
    async def _format_generic_response(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Safely get enable_markdown attribute with fallback
        enable_markdown = getattr(self, 'enable_markdown', True)
        
        return {
            'content': content,
            'message_type': message_type,
            'requires_markdown': enable_markdown,
            'is_streaming': False
        }
