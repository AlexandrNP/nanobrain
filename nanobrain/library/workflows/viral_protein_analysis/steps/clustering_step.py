...
class ClusteringStep(Step):
    ...
    def __init__(self, config: StepConfig, clustering_config: Optional[Dict[str, Any]] = None, **kwargs):
        ...
        self.clustering_mode = self.clustering_config.get(
            'mode', 'product_based')  # Default to product-based
    ...
    def _initialize_mmseqs2_tool(self):
        ...
        if tool_config_path.exists():
            ...
            self.nb_logger.info(
                f"✅ MMseqs2 tool loaded from workflow-local config: {tool_config_path}")
    ...
    async def process(self, input_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        # Add refactoring status to track framework compliance progress
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.nb_logger.info(
                f"🗂️ Starting clustering analysis in '{clustering_mode}' mode")
            ...
            if isinstance(curated_sequences_input, dict):
                ...
                self.nb_logger.info(
                    f"📊 Processing curated sequences from {len(curated_sequences_input)} protein classes")
                for protein_class, sequences in curated_sequences_input.items():
                    if isinstance(sequences, list):
                        all_sequences.extend(sequences)
                        self.nb_logger.debug(
                            f"Added {len(sequences)} sequences from class '{protein_class}'")
            elif isinstance(curated_sequences_input, list):
                # List format: [sequences]
                all_sequences = curated_sequences_input
                self.nb_logger.info(
                    f"📊 Processing curated sequences from list with {len(all_sequences)} sequences")
            else:
                self.nb_logger.warning(
                    f"⚠️ Unexpected curated_sequences format: {type(curated_sequences_input)}")
                all_sequences = []
            ...
            self.nb_logger.info(
                f"🔢 Total sequences for clustering: {len(all_sequences)}")

            # Debug: Check if sequences have aa_sequence field
            if all_sequences:
                sample_seq = all_sequences[0]
                self.nb_logger.info(
                    f"🔍 DEBUG: Sample sequence keys: {list(sample_seq.keys()) if isinstance(sample_seq, dict) else 'Not a dict'}")
                if isinstance(sample_seq, dict):
                    aa_seq = sample_seq.get('aa_sequence', '')
                    self.nb_logger.info(
                        f"🔍 DEBUG: Sample aa_sequence length: {len(aa_seq) if aa_seq else 'MISSING or EMPTY'}")
            ...
            if clustering_mode == 'product_based':
                ...
                execution_time = time.time() - step_start_time
                self.nb_logger.info(
                    f"✅ Product-based clustering completed in {execution_time:.2f} seconds")
                self.nb_logger.info(
                    f"Generated {len(clusters)} protein clusters")

                # Validate return format
                if not isinstance(clusters, dict):
                    raise ValueError(
                        f"Clustering step must return dict for protein_clusters, not {type(clusters)}")
            ...
            self.nb_logger.error(
                f"❌ Clustering failed: {str(e)}", exc_info=True)
    ...
    async def _execute_mmseqs2_clustering(self, all_sequences: List[Dict[str, Any]], step_start_time: float) -> Dict[str, Any]:
        ...
        self.nb_logger.info(
            f"🔧 Running MMseqs2 on {len(all_sequences)} sequences")
        ...
        execution_time = time.time() - step_start_time
        self.nb_logger.info(
            f"✅ MMseqs2 clustering completed in {execution_time:.2f} seconds")
    ...
    async def _cluster_by_product(self, sequences: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        for seq in sequences:
            ...
            product = seq.get('canonical_product') or seq.get(
        ...
        self.nb_logger.info(
            f"📦 Grouped sequences into {len(product_groups)} product categories")

        # Log synonym resolution impact
        synonym_resolved_count = sum(
            1 for seq in sequences if seq.get('synonym_resolved', False))
        if synonym_resolved_count > 0:
            self.nb_logger.info(
                f"🔗 {synonym_resolved_count} sequences had synonyms resolved")
            ...
            if len(product_sequences) >= self.min_cluster_size:
                ...
                for seq in product_sequences:
                    ...
                    if not genome_id:  # Handle empty strings
                        genome_id = 'unknown'
                    genome_distribution[genome_id] = genome_distribution.get(
                        genome_id, 0) + 1

                # Get full amino acid sequence for representative
                representative_sequence = representative_seq.get(
                    'aa_sequence', '')

                # Extract sequence IDs (same as member_sequences but named for response formatter)
                sequence_ids = [seq.get('patric_id', '')
                                for seq in product_sequences]
                ...
                has_synonyms = any(seq.get('synonym_resolved', False)
                                   for seq in product_sequences)
                ...
                cluster_num += 1
                self.nb_logger.debug(
                    f"Created cluster for '{product_name}': {len(product_sequences)} sequences")

        self.nb_logger.info(
            f"✅ Product-based clustering created {len(clusters)} clusters from {len(product_groups)} product groups")

        # Count clusters that benefited from synonym resolution
        synonym_enhanced_clusters = sum(
            1 for c in clusters.values() if c.get('has_synonym_resolved', False))
        if synonym_enhanced_clusters > 0:
            self.nb_logger.info(
                f"🔗 {synonym_enhanced_clusters} clusters contain synonym-resolved sequences")
        ...
        if clusters:
            ...
            self.nb_logger.info(
                f"🔍 DEBUG: Sample cluster '{sample_cluster_id}' keys: {list(sample_cluster.keys())}")
            self.nb_logger.info(
                f"🔍 DEBUG: length_stats: {sample_cluster.get('length_stats', 'MISSING')}")
            self.nb_logger.info(
                f"🔍 DEBUG: representative_sequence length: {len(sample_cluster.get('representative_sequence', ''))}")
            self.nb_logger.info(
                f"🔍 DEBUG: genome_distribution: {sample_cluster.get('genome_distribution', 'MISSING')}")
    ...
    async def _generate_llm_summary(self, clusters: Dict[str, Dict[str, Any]],
        ...
        try:
            ...
            max_clusters = self.clustering_config.get(
                'max_clusters_to_display', 20)
            include_snippets = self.clustering_config.get(
            ...
            for i, (cluster_id, cluster_data) in enumerate(sorted_clusters[:max_clusters]):
                ...
                if include_snippets and cluster_data.get('sequences'):
                    ...
                    if aa_sequence:
                        ...
                        cluster_info['representative_id'] = rep_seq.get(
                            'patric_id', '')

                cluster_summary_data.append(cluster_info)

            # Create prompt for LLM
            prompt = self._create_clustering_summary_prompt(
                cluster_summary_data, clustering_analysis)
            ...
            self.nb_logger.warning(
                f"⚠️ Failed to generate LLM summary: {e}", exc_info=True)
    ...
    def _create_clustering_summary_prompt(self, cluster_data: List[Dict],
        ...
        """
        total_clusters = clustering_analysis.get(
...
"""
    ...
    def _generate_fallback_summary(self, clusters: Dict[str, Dict[str, Any]],
        ...
        total_sequences = sum(cluster['cluster_size']
...
"""
    ...
    async def _create_complete_fasta(self, sequences: List[Dict[str, Any]]) -> str:
        ...
        self.nb_logger.info(
            f"🔧 Creating FASTA content for {len(sequences)} sequences")
        ...
        for i, seq in enumerate(sequences):
            try:
                ...
                if not aa_sequence:
                    self.nb_logger.debug(
                        f"Skipping sequence without aa_sequence: {patric_id}")
                    skipped_sequences += 1
                    continue

                if len(aa_sequence) < 10:  # Skip very short sequences
                    self.nb_logger.debug(
                        f"Skipping short sequence ({len(aa_sequence)} aa): {patric_id}")
                    skipped_sequences += 1
                    continue
                ...
                if not all(c.upper() in valid_aa_chars for c in aa_sequence):
                    self.nb_logger.debug(
                        f"Skipping sequence with invalid amino acids: {patric_id}")
                    skipped_sequences += 1
                    continue
                ...
                if valid_sequences <= 5:
                    self.nb_logger.debug(
                        f"FASTA entry {valid_sequences}: {patric_id} -> {product} ({len(aa_sequence)} aa)")

            except Exception as e:
                self.nb_logger.warning(
                    f"Error processing sequence {i}: {e}", exc_info=True)
                skipped_sequences += 1
                continue
        ...
        self.nb_logger.info(
            f"✅ Generated FASTA with {valid_sequences} valid sequences")
        self.nb_logger.info(
            f"⚠️ Skipped {skipped_sequences} invalid sequences")
        self.nb_logger.info(
            f"📊 FASTA content: {len(fasta_content):,} characters")
    ...
    async def _cache_complete_fasta(self, fasta_content: str):
        ...
        try:
            ...
            self.nb_logger.info(
                f"📊 Sequence count: {sequence_count:,} sequences")

        except Exception as e:
            self.nb_logger.warning(
                f"Failed to cache complete FASTA: {e}", exc_info=True)
    ...
    async def _convert_mmseqs2_results_to_clusters(self, clustering_report, original_sequences: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        self.nb_logger.info(
            f"Processing MMseqs2 clustering report: {clustering_report.total_clusters} clusters found")

        clusters = {}

        # Create mapping from sequences to enable cluster reconstruction
        sequence_map = {seq.get('patric_id', f'seq_{i}')
                                : seq for i, seq in enumerate(original_sequences)}
        ...
        if hasattr(clustering_report, 'clusters') and clustering_report.clusters:
            ...
            for i, cluster_data in enumerate(clustering_report.clusters):
                ...
                if cluster_members and len(cluster_members) >= self.min_cluster_size:
                    ...
                    products = [seq.get('product', 'unknown')
                                for seq in cluster_members]
                    consensus_product = max(
                        set(products), key=products.count) if products else 'unknown'
                    ...
                    representative_member = max(cluster_members, key=lambda x: len(
                        x.get('aa_sequence', ''))) if cluster_members else cluster_members[0]
                    representative_sequence = representative_member.get(
                        'aa_sequence', '')
            ...
            cluster_size = len(
                original_sequences) // clustering_report.total_clusters
        ...
        self.nb_logger.info(
            f"✅ Converted MMseqs2 results to {len(clusters)} clusters")
        return clusters
    ...
    async def _analyze_clusters(self, clusters: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        average_cluster_size = total_sequences_clustered / \
            total_clusters if total_clusters > 0 else 0

        # Cluster size distribution
        size_distribution = {}
        for size in cluster_sizes:
            size_range = self._get_size_range(size)
            size_distribution[size_range] = size_distribution.get(
                size_range, 0) + 1
        ...
        for cluster in clusters.values():
            ...
            consensus_product = cluster.get(
                'product_name', cluster.get('consensus_annotation', 'unknown'))
            size = cluster.get('size', cluster.get('cluster_size', 0))

            if consensus_product not in product_counts:
                product_counts[consensus_product] = {
                    'sequence_count': 0, 'cluster_count': 0}
        ...
        primary_method = max(clustering_methods.items(), key=lambda x: x[1])[
            0] if clustering_methods else 'unknown'
        ...
        self.nb_logger.info(
            f"  - Total sequences clustered: {total_sequences_clustered}")
        self.nb_logger.info(
            f"  - Average cluster size: {average_cluster_size:.2f}")
        self.nb_logger.info(
            f"  - Size range: {min(cluster_sizes) if cluster_sizes else 0} - {max(cluster_sizes) if cluster_sizes else 0}")
        self.nb_logger.info(
            f"  - Top products: {[p['product_name'] for p in top_products[:5]]}")
