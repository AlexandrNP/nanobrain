...
class BVBRCDataAcquisitionStep(Step):
    ...
    def _init_from_config(self, config: StepConfig, component_config: Dict[str, Any],
        ...
        if not agent_config or 'config_file' not in agent_config:
            raise ValueError(
                "SynonymDetection agent configuration required but not found")
        ...
        agent_class_path = agent_config_data['class']
        module_path, class_name = agent_class_path.rsplit('.', 1)
        module = import_module(module_path)
        agent_class = getattr(module, class_name)

        # Use direct from_config call with the configuration file path
        self.synonym_detection_agent = agent_class.from_config(
            agent_config_path)

        if hasattr(self, 'nb_logger') and self.nb_logger:
            self.nb_logger.info(
                f"🤖 SynonymDetection agent loaded from: {agent_config_path}")
    ...
    async def _validate_species_against_csv(self, species_name: str) -> bool:
        ...
        csv_path = getattr(self.config, 'csv_file_path',
                           'data/viral_analysis/BVBRC_genome_viral.csv')
        ...
        try:
            ...
            validation_result = validation_response.get(
                'content', '').strip().lower()

            # Interpret validation result
            is_valid = validation_result in [
                'yes', 'true', 'valid', 'found', 'present']

            if not is_valid:
                self.nb_logger.warning(
                    f"⚠️  Species '{species_name}' not found in CSV data")
                return False

            self.nb_logger.info(
                f"✅ Species '{species_name}' validated against CSV data")
            return True
    ...
    async def _get_virus_cache_key(self, input_params: Dict[str, Any]) -> str:
        ...
        try:
            ...
            self.nb_logger.error(
                f"❌ Virus species determination failed: {e}", exc_info=True)
    ...
    async def _execute_business_logic(self, input_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        target_genus = input_params.get(
            'target_genus') or input_params.get('organism')
        if not target_genus:
            raise ValueError(
        ...
        try:
            ...
            if filtered_proteins_path.exists():
                ...
                virus_species = input_params.get(
                    'virus_species') or input_params.get('organism')
                ...
                self.nb_logger.info(
                    f"🔄 Step 1A: Searching CSV file for genome IDs matching '{virus_species}'")
                csv_genome_ids = await self._search_bvbrc_csv_for_organism(virus_species)

                if csv_genome_ids:
                    # Use CSV search results
                    self.nb_logger.info(
                        f"✅ Found {len(csv_genome_ids)} genome IDs in CSV file")
                    self.nb_logger.info(
                        f"🔄 Step 1B: Downloading {len(csv_genome_ids)} genomes from CSV search results")
                    original_genomes = await self._download_genomes_by_ids(csv_genome_ids)
                    self.nb_logger.info(
                        f"✅ Downloaded {len(original_genomes)} genomes from CSV search")
                    ...
                    # Lineage-name BV-BRC queries are disabled by requirement.
                    # Do not use taxon_lineage_names or genus-based downloads.
                    raise ValueError(
                        "Lineage-name BV-BRC queries are disabled. Provide 'virus_species' to resolve via CSV/taxon_id, "
                        "or supply 'genome_ids' or 'taxon_id' explicitly."
                    )
    ...
    async def _download_genomes_by_genus(self, target_genus: str) -> List[GenomeData]:
        ...
        try:
            ...
            self.nb_logger.error(
                f"Failed to download genomes for genus {target_genus}: {e}", exc_info=True)
            raise
    ...
    async def _parse_genome_data(self, raw_data: bytes) -> List[GenomeData]:
        ...
        for line_num, line in enumerate(lines[1:], 2):
            try:
                ...
                self.nb_logger.warning(
                    f"Error parsing genome data at line {line_num}: {e}", exc_info=True)
                continue
    ...
    async def _search_bvbrc_csv_for_organism(self, target_organism: str) -> List[str]:
        ...
        try:
            ...
            csv_file_path = self.step_config.get(
            ...
            if not data_file.exists():
                if hasattr(self, 'nb_logger') and self.nb_logger:
                    self.nb_logger.warning(
                        f"BVBRC CSV file not found: {data_file}")
            ...
            if 'Genome Name' in df.columns:
                matching_entries = df[df['Genome Name'].str.contains(
                    target_organism, case=False, na=False)]
            ...
            if matching_entries is None or len(matching_entries) == 0:
                ...
                if 'chikungunya' in target_lower:
                    ...
                    if hasattr(self, 'nb_logger') and self.nb_logger:
                        self.nb_logger.info(
                            "🎯 Using representative Chikungunya virus genomes to leverage existing cache")
                    representative_chikv_ids = [
                        '1529938.3', '1534554.3', '1534555.3', '1534556.3', '1534557.3',  # Full-length genomes
                        # Additional representatives
                        '1534558.3', '1534559.3', '1534560.3', '1534561.3', '1534562.3'
                    ]
                    # Filter to only include IDs that exist in the CSV
                    if 'Genome ID' in df.columns:
                        available_ids = df['Genome ID'].astype(str).tolist()
                        working_ids = [
                            gid for gid in representative_chikv_ids if gid in available_ids]
                        if working_ids:
                            if hasattr(self, 'nb_logger') and self.nb_logger:
                                self.nb_logger.info(
                                    f"✅ Using {len(working_ids)} representative Chikungunya genomes")
                            return working_ids
                ...
                for term in search_terms:
                    if 'Genome Name' in df.columns:
                        term_matches = df[df['Genome Name'].str.contains(
            ...
            if matching_entries is not None and len(matching_entries) > 0 and 'Size' in matching_entries.columns:
                ...
                matching_entries_copy['Size_numeric'] = pd.to_numeric(
                    matching_entries_copy['Size'], errors='coerce')
                ...
                if len(full_length) > 0:
                    ...
                    if hasattr(self, 'nb_logger') and self.nb_logger:
                        self.nb_logger.info(
                            f"🔍 Filtered to {len(full_length)} full-length genomes (>= 5000 bp)")
                else:
                    if hasattr(self, 'nb_logger') and self.nb_logger:
                        self.nb_logger.warning(
                            f"⚠️ No full-length genomes found, using all {len(matching_entries)} matches")
            ...
            if matching_entries is not None and len(matching_entries) > 0:
                if 'Genome ID' in matching_entries.columns:
                    ...
                    genome_ids = matching_entries['Genome ID'].dropna().astype(
                        str).tolist()
                    # Remove any 'nan' strings that might have slipped through
                    genome_ids = [
                        gid for gid in genome_ids if gid != 'nan' and '.' in gid]

                    if hasattr(self, 'nb_logger') and self.nb_logger:
                        self.nb_logger.info(
                            f"🔍 Found {len(genome_ids)} genome IDs for '{target_organism}'")
                        if genome_ids:
                            self.nb_logger.info(
                                f"📋 Sample genome IDs: {genome_ids[:5]}")

                    return genome_ids

            if hasattr(self, 'nb_logger') and self.nb_logger:
                self.nb_logger.info(
                    f"🔍 No genomes found for '{target_organism}' in BVBRC CSV")
            return []
    ...
    async def _download_genomes_by_ids(self, genome_ids: List[str]) -> List[GenomeData]:
        ...
        if hasattr(self, 'nb_logger') and self.nb_logger:
            self.nb_logger.info(
                f"📥 Downloading {len(genome_ids)} genomes by specific IDs")
        ...
        for genome_id in genome_ids:
            try:
                ...
                if hasattr(self, 'nb_logger') and self.nb_logger:
                    self.nb_logger.warning(
                        f"Failed to download genome {genome_id}: {e}")
                continue

        if hasattr(self, 'nb_logger') and self.nb_logger:
            self.nb_logger.info(
                f"✅ Successfully downloaded {len(genomes)} genomes from {len(genome_ids)} requested IDs")
