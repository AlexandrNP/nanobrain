...
class AlignmentStep(Step):
    ...
    def _initialize_tools(self, config: StepConfig, alignment_config: Optional[Dict[str, Any]] = None):
        ...
        if tool_config_path.exists():
            ...
            try:
                ...
                if hasattr(self, 'nb_logger') and self.nb_logger:
                    self.nb_logger.info(
                        f"✅ MUSCLE tool loaded from workflow-local config: {tool_config_path}")

                # Extract alignment parameters from tool config
                self.alignment_tool = 'muscle'
                self.min_cluster_size = tool_config_dict.get(
                    'min_cluster_size', 3)
                self.max_cluster_size = tool_config_dict.get(
                    'max_cluster_size', 1000)

            except ImportError:
                if hasattr(self, 'nb_logger') and self.nb_logger:
                    self.nb_logger.warning(
                        "⚠️ MUSCLETool not available, using placeholder implementation")
            ...
            self.muscle_tool = None
            step_config_dict = config.config if hasattr(
                config, 'config') else {}
            if alignment_config:
                step_config_dict.update(alignment_config)

            self.alignment_config = step_config_dict.get(
                'alignment_config', {})
            self.alignment_tool = step_config_dict.get(
                'alignment_tool', 'muscle')
            self.min_cluster_size = step_config_dict.get('min_cluster_size', 3)
            self.max_cluster_size = step_config_dict.get(
                'max_cluster_size', 1000)
        ...
        if hasattr(self, 'nb_logger') and self.nb_logger:
            self.nb_logger.info(
                f"🧬 AlignmentStep initialized with tool: {self.alignment_tool}")

    async def process(self, input_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        """
        Process alignment step - ALL business logic here.

        Performs multiple sequence alignment on clustered protein sequences
        using the configured alignment tool (MUSCLE, ClustalW, etc.).
        This method executes within the configured executor environment.
        """
        self.nb_logger.info("🔄 Starting alignment step processing")

        # BUSINESS LOGIC STARTS HERE - Moved from execute() method
        ...
        try:
            ...
            self.nb_logger.info(
                f"✅ Alignment completed in {execution_time:.2f} seconds")
            self.nb_logger.info(
                f"Aligned {len(aligned_clusters)} protein clusters")
            ...
            self.nb_logger.error(f"❌ Alignment failed: {e}", exc_info=True)
            raise

    # REMOVED: execute() method override
    # ✅ Framework compliance restored: All business logic moved to process() method
    # ✅ BaseStep.execute() now handles executor environment automatically
        ...
        for cluster in clusters:
            ...
            if len(cluster_members) >= 3:  # Minimum sequences for meaningful alignment
                self.nb_logger.debug(
                    f"Aligning cluster {cluster.get('id')} with {len(cluster_members)} sequences")
                ...
                self.nb_logger.warning(
                    f"Cluster {cluster.get('id')} has too few sequences ({len(cluster_members)}) for alignment")
    ...
    async def _align_cluster_sequences(self, cluster: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.muscle_tool and hasattr(self.muscle_tool, 'align_sequences'):
            self.nb_logger.debug(
                f"🔧 Using real MUSCLETool for cluster {cluster.get('id')}")
            try:
                # Use real MUSCLE tool via from_config
                return await self.muscle_tool.align_sequences(cluster)
            except Exception as e:
                self.nb_logger.warning(
                    f"⚠️ MUSCLETool failed for cluster {cluster.get('id')}, falling back to placeholder: {e}", exc_info=True)
                return await self._placeholder_alignment(cluster)
        else:
            # Fallback to placeholder implementation
            self.nb_logger.debug(
                f"🔧 Using placeholder alignment for cluster {cluster.get('id')}")
            return await self._placeholder_alignment(cluster)
    ...
    async def _placeholder_alignment(self, cluster: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if cluster_members and 'aa_sequence' in cluster_members[0]:
            ...
            avg_length = sum(len(member.get('aa_sequence', ''))
                             for member in cluster_members) / len(cluster_members)
