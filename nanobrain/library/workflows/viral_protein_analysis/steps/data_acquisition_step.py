...
class BVBRCDataAcquisitionStep(Step):
    ...
    def _initialize_tools(self, config: StepConfig, bvbrc_config: Optional[Dict[str, Any]] = None):
        ...
        if hasattr(config, 'resolved_tools') and config.resolved_tools:
            ...
            taxonomic_agent = config.resolved_tools.get(
        ...
        if tool_config_path.exists():
            ...
            if hasattr(self, 'nb_logger') and self.nb_logger:
                self.nb_logger.info(
                    f"✅ BV-BRC tool loaded from workflow-local config: {tool_config_path}")

        else:
            # Final fallback - should not happen with proper configuration
            if hasattr(self, 'nb_logger') and self.nb_logger:
                self.nb_logger.warning(
                    f"⚠️ Workflow-local tool config not found: {tool_config_path}")
                self.nb_logger.error(
                    "❌ No BV-BRC tool available - enhanced resolution failed and no local config found")

            raise ValueError(
                "BV-BRC tool configuration required but not available through enhanced or legacy systems")
    ...
    def _finalize_tool_initialization(self, config: StepConfig):
        ...
        self.timeout_seconds = getattr(
            config, 'timeout_seconds', 1200)  # 20 minutes
        ...
        if hasattr(self, 'nb_logger') and self.nb_logger:
            self.nb_logger.info(
                f"🧬 BVBRCDataAcquisitionStep initialized with tool: {type(self.bv_brc_tool).__name__}")
            self.nb_logger.info(
                f"⏱️ Timeout set to {self.timeout_seconds} seconds (20 minutes)")
    ...
    async def _extract_virus_species_with_agent(self, user_query: str) -> str:
        ...
        try:
            ...
            response = await self.species_validation_agent.process(extraction_request)
            ...
            if not species_name or species_name == "UNKNOWN_SPECIES":
                raise ValueError(
                    f"Agent could not extract virus species from query: '{user_query}'")

            self.nb_logger.info(
                f"🧬 Agent extracted virus species: '{species_name}' from query: '{user_query}'")
            return species_name

        except Exception as e:
            self.nb_logger.error(
                f"❌ Virus species extraction failed: {e}", exc_info=True)
            raise ValueError(
                f"Cannot extract virus species from query: '{user_query}'. Error: {e}")
    ...
    async def _validate_species_against_csv(self, species_name: str) -> bool:
        ...
        try:
            # Try pandas first, but fallback to manual parsing if it fails
            try:
                import pandas as pd
                df = pd.read_csv(csv_path, sep='\t')
                available_species = df['genome.genome_name'].unique().tolist()
            except (ImportError, AttributeError) as pandas_error:
                self.nb_logger.warning(
                    f"⚠️  Pandas unavailable ({pandas_error}), using fallback TSV parsing")
                # Fallback: manual TSV parsing
                available_species = self._parse_tsv_manually(csv_path)

            # Direct string matching validation (more reliable than agent-based)
            # Check if the species name appears in any of the available species
            is_valid = any(species_name.lower() in available_species_name.lower()
                           for available_species_name in available_species)

            if not is_valid:
                self.nb_logger.warning(
                    f"⚠️  Species '{species_name}' not found in TSV data")
                self.nb_logger.info(
                    f"🔍 Available species sample: {available_species[:5]}")
                return False

            self.nb_logger.info(
                f"✅ Species '{species_name}' validated against TSV data")
            return True

        except Exception as e:
            self.nb_logger.error(
                f"❌ CSV validation failed: {e}", exc_info=True)
            return False

    def _parse_tsv_manually(self, csv_path: str) -> List[str]:
        """Fallback method to parse TSV file manually when pandas is unavailable"""
        available_species = []
        try:
            ...
            if not lines:
                self.nb_logger.warning(f"⚠️  TSV file {csv_path} is empty")
                return []

            # Find the header and locate the 'genome.genome_name' column
            ...
            for line in lines[1:]:  # Skip header
                ...
                if len(columns) > name_column_index:
                    species_name = columns[name_column_index].strip()
                    if species_name and species_name not in available_species:
                        available_species.append(species_name)

            self.nb_logger.info(
                f"📄 Manually parsed {len(available_species)} unique species from TSV")
            return available_species

        except Exception as e:
            self.nb_logger.error(
                f"❌ Manual TSV parsing failed: {e}", exc_info=True)
            return []

    async def _get_virus_cache_key(self, input_params: Dict[str, Any]) -> str:
        """Generate cache key using virus species from input parameters with CSV validation"""

        # First, try to get virus species directly from input parameters (harmonized input)
        species_name = (input_params.get('virus_species') or
                        input_params.get('target_genus') or
                        input_params.get('organism'))

        # If no direct species parameter, try to extract from user query using agent
        if not species_name:
            user_query = input_params.get('user_query', '')
            if not user_query:
                # Try other query sources
                user_query = input_params.get(
                    'target_organism', '') or input_params.get('organism', '')

            if not user_query:
                raise ValueError(
                    "Cannot determine virus species: no species parameter or query data provided. "
                    "Please provide 'virus_species', 'target_genus', 'organism', or 'user_query' parameter."
                )

            # Extract species using agent
            species_name = await self._extract_virus_species_with_agent(user_query)
        else:
            self.nb_logger.info(
                f"🧬 Using virus species from input parameters: '{species_name}'")
        ...
        self.nb_logger.info(
            f"💾 Generated cache key: '{cache_key}' for species: '{species_name}'")
        return cache_key
    ...
    async def process(self, input_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        # Extract parameters from input_data - Handle NanoBrain data unit wrapping
        # NanoBrain passes data wrapped in data unit name (e.g., {'synonyms_input': actual_data})
        if 'synonyms_input' in input_data and isinstance(input_data['synonyms_input'], dict):
        ...
        # Debug: Log what parameters are actually received
        self.nb_logger.info(
        ...
        if ('target_genus' not in input_params and
            'organism' not in input_params and
                'virus_species' not in input_params):
            raise ValueError(
                "Data acquisition requires 'target_genus', 'organism', or 'virus_species' parameter. "
                "Please specify the target virus genus, organism, or species for data acquisition."
            )
    ...
    async def execute(self, input_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        target_genus = (input_params.get('target_genus') or
                        input_params.get('organism') or
                        input_params.get('virus_species'))
        if not target_genus:
            raise ValueError(
                "No target genus, organism, or virus species specified. "
                "Please provide 'target_genus', 'organism', or 'virus_species' parameter for data acquisition."
            )
        ...
        try:
            ...
            cached_proteins_path = self._get_cached_file_path(
                cache_key, "proteins")
            filtered_proteins_path = self._get_cached_file_path(
                cache_key, "proteins.filtered")
            ...
            if filtered_proteins_path.exists():
                with open(filtered_proteins_path, 'r') as f:
                    ...
                    if line_count <= 1:  # Only header or empty
                        self.nb_logger.warning(
                            f"🔄 Current cache {cache_key} is empty, searching for existing good cache...")
                        ...
                        for cache_file in cache_dir.glob("*_proteins.filtered.tsv"):
                            if cache_file != filtered_proteins_path:
                                with open(cache_file, 'r') as f:
                                    existing_count = sum(
                                        1 for _ in f) - 1  # Subtract header
                                    if existing_count > 30000:  # Good cache with substantial data
                                        existing_cache_key = cache_file.stem.replace(
                                            '_proteins.filtered', '')
                                        self.nb_logger.info(
                                            f"✅ Found good cache {existing_cache_key} with {existing_count:,} proteins, using instead")
            ...
            if filtered_proteins_path.exists():
                ...
                print(
                    f"💾 FOUND EXISTING FILTERED PROTEINS CACHE: {cached_proteins_path}")
                self.nb_logger.info(
                    f"💾 Found existing filtered proteins cache: {cached_proteins_path}")
                ...
                print(
                    f"💾 FOUND EXISTING PROTEINS CACHE: {cached_proteins_path}")
                self.nb_logger.info(
                    f"💾 Found existing proteins cache: {cached_proteins_path}")
                ...
                if 'genome_ids' in input_params and input_params['genome_ids']:
                    ...
                    if 'taxon_id' in input_params:
                        ...
                        self.nb_logger.info(
                            f"🔄 Step 1: Downloading {len(genome_ids)} specific genomes from taxon_id {taxon_id}")
                        ...
                        self.nb_logger.info(
                            f"🔄 Step 1: Downloading {len(genome_ids)} specific genomes from BV-BRC")
                        original_genomes = await self._download_specific_genomes(genome_ids)

                    self.nb_logger.info(
                        f"✅ Downloaded {len(original_genomes)} specific genomes")

                elif 'taxon_id' in input_params:
                    # Download by taxon_id
                    taxon_id = input_params['taxon_id']
                    self.nb_logger.info(
                        f"🔄 Step 1: Downloading all genomes for taxon_id {taxon_id} from BV-BRC")
                    original_genomes = await self._download_genomes_by_taxon_id(taxon_id)
                    self.nb_logger.info(
                        f"✅ Downloaded {len(original_genomes)} genomes for taxon_id {taxon_id}")

                else:
                    # Download by genus - use the target_genus already resolved from input parameters
                    # Use the resolved target_genus from above
                    target_genus_for_download = target_genus
                    if not target_genus_for_download:
                        raise ValueError(
                            "No target genus specified for genome download. "
                            "Please provide 'target_genus', 'organism', or 'virus_species' parameter."
                        )
                    self.nb_logger.info(
                        f"🔄 Step 1: Downloading {target_genus_for_download} genomes from BV-BRC")
                    original_genomes = await self._download_genomes_by_genus(target_genus_for_download)
                    self.nb_logger.info(
                        f"✅ Downloaded {len(original_genomes)} {target_genus_for_download} genomes")
            ...
            self.nb_logger.info(
                f"✅ Found {len(unique_proteins)} unique proteins")
            ...
            self.nb_logger.info(
                f"✅ Retrieved {len(protein_sequences)} protein sequences")

            # Step 6: Extract annotations from protein data (already available from p3-get-genome-features)
            self.nb_logger.info(
                "🔄 Step 6: Extracting protein annotations from existing data")
            protein_annotations = await self._extract_annotations_from_proteins(unique_proteins)
            self.nb_logger.info(
                f"✅ Extracted annotations for {len(protein_annotations)} proteins")

            # Step 7: Create annotated FASTA with patric_id and product information
            self.nb_logger.info(
                "🔄 Step 7: Creating annotated FASTA file with patric_id and product information")
            annotated_fasta = await self._create_annotated_fasta_with_cache(protein_sequences, protein_annotations, cache_key)
            self.nb_logger.info(
                "✅ Created annotated FASTA file with enhanced headers")

            execution_time = time.time() - step_start_time
            self.nb_logger.info(
                f"🎉 BV-BRC data acquisition completed in {execution_time:.2f} seconds")
            ...
            if not original_genomes and not filtered_genomes and unique_proteins:
                self.nb_logger.info(
                    "🔄 Reconstructing genome data from cached protein information")
                ...
                self.nb_logger.info(
                    f"✅ Reconstructed {len(reconstructed_genomes)} genome entries from cached protein data")
            ...
            self.nb_logger.error(
                f"❌ BV-BRC data acquisition failed: {e}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'execution_time': time.time() - step_start_time
    ...
    async def _download_specific_genomes(self, genome_ids: List[str]) -> List[GenomeData]:
        ...
        for i in range(0, len(genome_ids), batch_size):
            ...
            if result.returncode != 0:
                raise RuntimeError(
                    f"p3-all-genomes command failed: {result.stderr_text}")

            batch_genomes = await self._parse_genome_data(result.stdout)
            all_genomes.extend(batch_genomes)

        self.nb_logger.info(
            f"📦 Retrieved {len(all_genomes)}/{len(genome_ids)} requested genomes by ID")
    ...
    async def _download_specific_genomes_by_taxon_id(self, genome_ids: List[str], taxon_id: str) -> List[GenomeData]:
        ...
        self.nb_logger.info(
            f"📦 Retrieved {len(filtered_genomes)}/{len(genome_ids)} requested genomes from taxon_id {taxon_id}")
    ...
    async def _download_genomes_by_taxon_id(self, taxon_id: str) -> List[GenomeData]:
        ...
        if not await self.bv_brc_tool.verify_installation():
            raise RuntimeError(
                "BV-BRC tool is not accessible. Please check installation.")
        ...
        try:
            ...
            if result.returncode != 0:
                raise RuntimeError(
                    f"p3-all-genomes command failed: {result.stderr.decode()}")

            genomes = await self._parse_genome_data(result.stdout)

            if not genomes:
                self.nb_logger.warning(
                    f"No genomes found for taxon_id {taxon_id} in BV-BRC")

            return genomes

        except Exception as e:
            self.nb_logger.error(
                f"Failed to download genomes for taxon_id {taxon_id}: {e}", exc_info=True)
            raise
    ...
    async def _download_genomes_by_genus(self, target_genus: str) -> List[GenomeData]:
        ...
        try:
            ...
            if not genomes:
                self.nb_logger.warning(
                    f"No genomes found for genus {target_genus} in BV-BRC")

            return genomes

        except Exception as e:
            self.nb_logger.error(
                f"Failed to download genomes for genus {target_genus}: {e}")
            raise
    ...
    async def _parse_genome_data(self, raw_data: bytes) -> List[GenomeData]:
        ...
        if len(lines) < 2:  # Must have header + at least one data line
            self.nb_logger.warning(
                "BV-BRC returned only headers, no genome data")
            return []
        ...
        try:
            ...
            if genome_id_idx is None:
                raise ValueError(
                    f"genome_id column not found in headers: {header}")
            if genome_name_idx is None:
                raise ValueError(
                    f"genome_name column not found in headers: {header}")
            if genome_length_idx is None:
                raise ValueError(
                    f"genome_length column not found in headers: {header}")
        ...
        for line_num, line in enumerate(lines[1:], 2):
            try:
                ...
                try:
                    ...
                    self.nb_logger.warning(
                        f"Invalid genome length '{genome_length_str}' for {genome_id}")
                    continue
                ...
                self.nb_logger.warning(
                    f"Error parsing genome data at line {line_num}: {e}")
                continue
        ...
        for genome in genomes:
            if self.min_genome_length <= genome.genome_length <= self.max_genome_length:
                ...
                self.nb_logger.debug(
                    f"Filtered genome {genome.genome_id}: length {genome.genome_length} outside range")
    ...
    async def _get_unique_protein_md5s(self, genome_ids: List[str], cache_key: str = None) -> List[ProteinData]:
        ...
        self.nb_logger.info(
            f"Processing all {len(genome_ids)} genomes in single operation")
        ...
        self.nb_logger.info(
            f"Deduplicated {len(all_proteins)} proteins to {len(unique_proteins)} unique MD5s")
    ...
    async def _get_proteins_for_genomes(self, genome_ids: List[str], cache_key: str = None) -> List[ProteinData]:
        ...
        if filtered_proteins_path.exists():
            ...
            self.nb_logger.info(
                f"🗂️ Using filtered proteins cache: {cached_proteins_path}")
        else:
            print(f"🗂️ PROTEINS CACHE PATH: {cached_proteins_path}")
            self.nb_logger.info(
                f"🗂️ Proteins cache file: {cached_proteins_path}")
        ...
        if cached_proteins_path.exists():
            ...
            try:
                ...
                self.nb_logger.info(
                    f"✅ Loaded {len(proteins)} proteins from cache")
                return proteins
            except Exception as e:
        ...
        try:
            ...
            self.nb_logger.info(
                f"🔄 Step 1: Running p3-all-genomes for {len(genome_ids)} genomes (timeout: {self.timeout_seconds}s)")
            ...
            if process1.returncode != 0:
                self.nb_logger.error(
                    f"p3-all-genomes failed: {stderr1.decode()}")
                return []
            ...
            self.nb_logger.info(
                f"🔄 Step 2: MANDATORY pipeline - cut -f1 | p3-get-genome-features (timeout: {self.timeout_seconds}s)")
            ...
            if process2.returncode != 0:
                self.nb_logger.error(
                    f"Pipeline command failed: {stderr2.decode()}")
                return []
            ...
            self.nb_logger.info(
                f"✅ Successfully extracted {len(proteins)} proteins using MANDATORY pipeline")
            self.nb_logger.info(
                f"💾 Protein data cached to: {cached_proteins_path}")

            return proteins

        except asyncio.TimeoutError:
            self.nb_logger.error(
                f"❌ BV-BRC pipeline timed out after {self.timeout_seconds} seconds")
            return []
        except Exception as e:
    ...
    async def _parse_protein_data(self, raw_data: bytes) -> List[ProteinData]:
        ...
        try:
            ...
            if patric_id_idx is None:
                raise ValueError(
                    f"patric_id column not found in headers: {header}")
            if md5_idx is None:
                raise ValueError(
                    f"aa_sequence_md5 column not found in headers: {header}")
            if genome_id_idx is None:
                raise ValueError(
                    f"genome_id column not found in headers: {header}")
    ...
    async def _get_feature_sequences(self, protein_data: List[ProteinData]) -> List[SequenceData]:
        ...
        self.nb_logger.info(
            f"Getting sequences for all {len(protein_data)} proteins in single operation")
    ...
    async def _get_sequences_for_proteins(self, protein_batch: List[ProteinData]) -> List[SequenceData]:
        ...
        if hasattr(self, 'current_cache_key') and self.current_cache_key:
            ...
            cached_sequences_file = self._get_cached_file_path(
                self.current_cache_key, "sequences.fasta")

            # Also check for existing annotated FASTA file
            annotated_fasta_file = self._get_cached_file_path(
                self.current_cache_key, "proteins_annotated.fasta")
            ...
            if cached_sequences_file.exists():
                ...
                print(
                    f"💾 FOUND EXISTING RAW SEQUENCES CACHE: {sequence_cache_file}")
            elif annotated_fasta_file.exists():
                sequence_cache_file = annotated_fasta_file
                print(
                    f"💾 FOUND EXISTING ANNOTATED FASTA CACHE: {sequence_cache_file}")

            if sequence_cache_file:
                self.nb_logger.info(
                    f"💾 Found existing sequence cache: {sequence_cache_file}")
                ...
                try:
                    ...
                    patric_to_md5 = {
                        protein.patric_id: protein.aa_sequence_md5 for protein in protein_batch}

                    # Debug: Log sample patric_ids from both sources
                    if raw_sequences and patric_to_md5:
                        sample_fasta_ids = [
                            seq.get('patric_id', '') for seq in raw_sequences[:5]]
                        sample_protein_ids = list(patric_to_md5.keys())[:5]
                        self.nb_logger.info(
                            f"🔍 DEBUG: Sample FASTA patric_ids: {sample_fasta_ids}")
                        self.nb_logger.info(
                            f"🔍 DEBUG: Sample protein patric_ids: {sample_protein_ids}")

                        # Check for exact matches
                        fasta_id_set = {seq.get('patric_id', '')
                                        for seq in raw_sequences}
                        protein_id_set = set(patric_to_md5.keys())
                        matches = fasta_id_set.intersection(protein_id_set)
                        self.nb_logger.info(
                            f"🔍 DEBUG: Found {len(matches)} exact patric_id matches out of {len(fasta_id_set)} FASTA and {len(protein_id_set)} protein IDs")
                        ...
                        if len(matches) == 0 and len(fasta_id_set) > 0 and len(protein_id_set) > 0:
                            ...
                            self.nb_logger.warning(
                                f"⚠️ No patric_id matches! FASTA format: '{sample_fasta}', Protein format: '{sample_protein}'")
                    ...
                    print(
                        f"✅ LOADED {len(sequences)} SEQUENCES FROM CACHE - SKIPPING p3-get-feature-sequence")
                    self.nb_logger.info(
                        f"💾 Loaded {len(sequences)} sequences from cache - skipping p3-get-feature-sequence")
                    return sequences
                    ...
                    self.nb_logger.warning(
                        f"⚠️ Failed to load cached sequences: {e}")
        ...
        if hasattr(self, 'current_cache_key') and self.current_cache_key:
            ...
            filtered_file = self._get_cached_file_path(
                self.current_cache_key, "proteins.filtered")
            if filtered_file.exists():
                cached_proteins_file = filtered_file
            else:
                cached_proteins_file = self._get_cached_file_path(
                    self.current_cache_key, "proteins")
        ...
        if not cached_proteins_file or not cached_proteins_file.exists():
            self.nb_logger.error(
                "No cached proteins file found from p3-get-genome-features")
            return []

        print(
            f"🗂️ USING PROTEINS FILE FROM p3-get-genome-features: {cached_proteins_file}")
        self.nb_logger.info(
            f"🗂️ Using proteins file from p3-get-genome-features: {cached_proteins_file}")
        ...
        try:
            ...
            if "filtered" in str(cached_proteins_file):
                ...
                print(
                    f"🔧 USING EXISTING FILTERED PROTEINS FILE: {filtered_proteins_file}")
                self.nb_logger.info(
                    f"🔧 Using existing filtered proteins file: {filtered_proteins_file}")
            else:
                ...
                filtered_proteins_file = cached_proteins_file.with_suffix(
                    '.filtered.tsv')
                ...
                print(
                    f"🔧 CREATED FILTERED PROTEINS FILE: {filtered_proteins_file}")
                self.nb_logger.info(
                    f"🔧 Created filtered proteins file: {filtered_proteins_file}")
            ...
            self.nb_logger.debug(
                f"Executing sequence command: {sequence_command}")
            ...
            try:
                ...
                await process.wait()
                raise RuntimeError(
                    f"p3-get-feature-sequence command timed out after {self.timeout_seconds} seconds")

            if process.returncode != 0:
                error_msg = stderr.decode() if stderr else "Unknown error"
                self.nb_logger.warning(
                    f"p3-get-feature-sequence failed: {error_msg}")
                return []
            ...
            if hasattr(self, 'current_cache_key') and self.current_cache_key:
                ...
                try:
                    ...
                    self.nb_logger.info(
                        f"💾 Cached sequences to: {cached_sequences_file}")
                except Exception as e:
                    self.nb_logger.warning(
                        f"⚠️ Failed to cache sequences: {e}")
            ...
            self.nb_logger.warning(
                f"Error getting sequences for protein batch: {e}")
            return []
    ...
    async def _parse_sequence_data_with_patric_id(self, raw_data: bytes) -> List[Dict[str, str]]:
        ...
        for line in lines:
            ...
            if line.startswith('>'):
                ...
                if current_header and current_sequence:
                    patric_id = self._extract_patric_id_from_fasta_header(
                        current_header)
                    if patric_id:
                        sequences.append({
                            'patric_id': patric_id,
        ...
        self.nb_logger.info(
            f"Parsed {len(sequences)} sequences from FASTA format")
        return sequences
    ...
    def _extract_patric_id_from_fasta_header(self, header: str) -> str:
        ...
        try:
            ...
            self.nb_logger.warning(
                f"Error extracting patric_id from header '{header}': {e}")
            return ""
    ...
    async def _parse_fasta_content(self, fasta_content: str) -> List[Dict[str, str]]:
        ...
        for line in lines:
            ...
            if line.startswith('>'):
                # Process previous sequence if exists
                if current_header and current_sequence:
                    patric_id = self._extract_patric_id_from_any_header(
                        current_header)
                    if patric_id:
                        sequences.append({
                            'patric_id': patric_id,
        ...
        self.nb_logger.info(
            f"Parsed {len(sequences)} sequences from cached FASTA content")
        return sequences
    ...
    async def _create_annotated_fasta_with_cache(self, sequences: List[SequenceData],
        ...
        fasta_cache_path = self._get_cached_file_path(
            cache_key, "proteins_annotated.fasta")
        ...
        if fasta_cache_path.exists():
            ...
            try:
                ...
                self.nb_logger.info(
                    f"✅ Loaded FASTA with {header_count} sequences from cache")
                return cached_fasta
        ...
        try:
            ...
            print(
                f"📊 FASTA cache file size: {file_size:,} bytes ({file_size/1024/1024:.1f} MB)")
            self.nb_logger.info(
                f"📊 FASTA cache file size: {file_size:,} bytes")
    ...
    async def _create_annotated_fasta(self, sequences: List[SequenceData],
        ...
        for seq in sequences:
            ...
            if protein and annotation:
                ...
                self.nb_logger.debug(
                    f"FASTA entry: {protein.patric_id} -> {annotation.product or protein.product}")
            else:
        ...
        if fasta_lines:
            sample_headers = [
                line for line in fasta_lines[:10] if line.startswith('>')]
    ...
    def _format_fasta_header_with_patric_id(self, patric_id: str, product: str, gene: str, md5: str) -> str:
        ...
        for comp in components:
            cleaned = str(comp).replace("|", "_").replace(
                "\t", " ").replace("\n", " ")
