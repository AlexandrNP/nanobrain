...
class SimplifiedAlphavirusWorkflow(Workflow):
    """
    Simplified Alphavirus Workflow with delegated business logic
    
    This workflow only orchestrates steps and delegates all business logic
    to individual step implementations. This addresses the original issue
    where the workflow class contained too much domain-specific logic.
    """
    ...
    def _init_from_config(self, config: WorkflowConfig, component_config: Dict[str, Any],
        ...
        self.virus_name = config_dict.get('virus_name', 'Chikungunya virus')
        ...
        self.workflow_logger.info(f"🧬 SimplifiedAlphavirusWorkflow initialized")
    ...
    async def execute(self, input_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            self.workflow_logger.info("🚀 Starting simplified Alphavirus workflow execution")
            ...
            self.workflow_logger.info(f"✅ Simplified workflow completed in {workflow_execution_time:.2f} seconds")
            ...
            workflow_execution_time = time.time() - workflow_start_time
            self.workflow_logger.error(f"❌ Simplified workflow failed: {e}")
    ...
    def _get_file_type_configurations(self) -> Dict[str, Any]:
        ...
        return {
            'filtered_genomes': {
                'filename': 'alphavirus_filtered_genomes.json',
                'format': 'json'
            },
            'unique_proteins': {
                'filename': 'alphavirus_unique_proteins.fasta',
                'format': 'fasta'
            },
            'clusters': {
                'filename': 'alphavirus_clusters.json',
                'format': 'json'
            },
            'pssm_matrices': {
                'filename': 'alphavirus_pssm_matrices.json',
                'format': 'json'
            },
            'viral_pssm_json': {
