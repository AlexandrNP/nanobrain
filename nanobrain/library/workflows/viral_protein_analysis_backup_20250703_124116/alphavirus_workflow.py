...
class WorkflowData:
    ...
    def update_from_clustering(self, result: Dict[str, Any]) -> None:
        ...
        # CRITICAL FIX: Use 'protein_clusters' key as returned by clustering step
        self.clusters = result.get('protein_clusters', {})
...
class AlphavirusWorkflow(Workflow):
    ...
    def extract_component_config(cls, config: WorkflowConfig) -> Dict[str, Any]:
        ...
        return {
            ...
            'processing_mode': 'sequential',  # Required for step base
            'add_metadata': True  # Required for step base
        }
    ...
    async def execute_full_workflow(self, input_params: Optional[Dict[str, Any]] = None) -> WorkflowResult:
        ...
        # Validate input parameters for alphavirus processing
        if input_params and 'genome_ids' in input_params:
            self.workflow_logger.info(f"🎯 Specific genome workflow: {len(input_params['genome_ids'])} genome IDs")
        ...
        # Handle different input parameter patterns
        if 'genome_ids' in input_params:
            workflow_params['genome_ids'] = input_params['genome_ids']
            workflow_params['analysis_focus'] = input_params.get('analysis_focus', 'pssm_generation')
        elif 'taxon_id' in input_params:
            workflow_params['taxon_id'] = input_params['taxon_id']
            workflow_params['analysis_focus'] = input_params.get('analysis_focus', 'pssm_generation')
        else:
            # Default to genus-based analysis
        ...
        try:
            ...
            # Validate genome processing if specific genomes were requested
            if workflow_params and 'genome_ids' in workflow_params:
                processed_count = len(self.workflow_data.filtered_genomes) if hasattr(self.workflow_data, 'filtered_genomes') else 0
                requested_count = len(workflow_params['genome_ids'])
                
                if processed_count != requested_count:
                    self.workflow_logger.warning(
                        f"⚠️ Processed {processed_count}/{requested_count} requested genomes"
                    )
    ...
    async def process(self, input_data: Dict[str, Any], **kwargs) -> Any:
        ...
        try:
            ...
            # Execute each step in order, passing accumulated results to each step
            # Use the actual configured step IDs from the workflow
            configured_steps = ['data_acquisition', 'annotation_mapping', 'sequence_curation', 'clustering_analysis']
            
            # Start with original input data
            current_step_input = input_data.copy()
            
            for step_id in configured_steps:
                if step_id in self.child_steps:
                    step = self.child_steps[step_id]
                    self.workflow_logger.info(f"🔄 Executing step: {step_id}")
                    
                    # Call execute method with accumulated input from previous steps
                    if hasattr(step, 'execute') and 'execute' in str(type(step.execute)):
                        step_result = await step.execute(current_step_input)
                    else:
                        # Fallback to process method for other step types
                        step_result = await step.process(current_step_input, **kwargs)
                    ...
                    # Update input for next step with current step's results
                    if step_result and step_result.get('success'):
                        # Merge step results into input for next step
                        current_step_input.update(step_result)
                        self.workflow_logger.debug(f"✅ Updated input for next step with {step_id} results")
                    else:
                        self.workflow_logger.warning(f"⚠️ Step {step_id} did not return successful results")
                    
                    # Update progress
                    progress = (len(step_results) * 90) // len(configured_steps) + 5  # 5% to 95%
                    if self.progress_reporter:
                        await self.progress_reporter.update_progress(
    ...
    def _update_workflow_data(self, workflow_data: WorkflowData, step_id: str, step_result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            elif step_id == 'clustering_analysis' and step_result.get('success'):
    ...
    async def _generate_viral_pssm_json(self, workflow_data: WorkflowData) -> Dict[str, Any]:
        ...
        try:
            # Debug log the clusters before adding to JSON
            if workflow_data.clusters:
                sample_cluster_id = next(iter(workflow_data.clusters.keys()))
                sample_cluster = workflow_data.clusters[sample_cluster_id]
                self.workflow_logger.info(f"🔍 DEBUG: Viral PSSM JSON - sample cluster '{sample_cluster_id}' keys: {list(sample_cluster.keys())}")
                self.workflow_logger.info(f"🔍 DEBUG: Sample cluster length_stats: {sample_cluster.get('length_stats', 'MISSING')}")
                self.workflow_logger.info(f"🔍 DEBUG: Sample cluster representative_sequence length: {len(sample_cluster.get('representative_sequence', ''))}")
...
async def create_alphavirus_workflow(config_path: Optional[str] = None,
                                   ...
                                   target_genus: Optional[str] = None,
    ...
    Args:
        ...
        taxon_id: NCBI Taxon ID for specific alphavirus species
        genome_ids: List of specific genome IDs to process
        analysis_type: Type of analysis to perform ('pssm_generation', 'comprehensive', etc.)
        target_genus: Target genus for analysis (default: 'Alphavirus')
    ...
    if hasattr(workflow_config, 'workflow_directory') and workflow_config.workflow_directory:
        ...
        if not Path(workflow_config.workflow_directory).is_absolute():
            # Make it relative to the workflow config file location's parent directory
            # config_path points to: .../viral_protein_analysis/config/AlphavirusWorkflow.yml
            # We want workflow_directory to point to: .../viral_protein_analysis/
            config_file_path = Path(config_path)
            workflow_root = config_file_path.parent.parent  # Go up from config/ to viral_protein_analysis/
            workflow_config.workflow_directory = str(workflow_root)
    ...
    if taxon_id:
        workflow_params['taxon_id'] = taxon_id
    if genome_ids:
        workflow_params['genome_ids'] = genome_ids
    if analysis_type:
        workflow_params['analysis_type'] = analysis_type
    if target_genus:
        workflow_params['target_genus'] = target_genus
