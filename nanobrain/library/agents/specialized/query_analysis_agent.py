...
class QueryAnalysisAgentConfig(AgentConfig):
    """
    Configuration schema for QueryAnalysisAgent

    Extends the base AgentConfig with specialized parameters for query analysis,
    intent classification, and domain identification.
    """

    # Query analysis specific configuration - inherit all AgentConfig fields
    # No need to redefine confidence_threshold as it's already in AgentConfig
    analysis_depth: str = Field(
        default="comprehensive",
        description="Analysis depth level: standard, detailed, comprehensive"
    )
    biological_context_focus: bool = Field(
        default=True,
        description="Enable biological domain specialization"
    )
    domain_specialization: Optional[Dict[str, bool]] = Field(
        default=None,
        description="Domain-specific analysis capabilities"
    )
    intent_categories: Optional[list] = Field(
        default=None,
        description="Supported intent classification categories"
    )
    parameter_types: Optional[Dict[str, bool]] = Field(
        default=None,
        description="Types of parameters to extract from queries"
    )
...
class QueryAnalysisAgent(ConversationalSpecializedAgent):
    ...
    **Research Applications:**
    ...
    def _get_config_class(cls):
        """Return the appropriate config class for QueryAnalysisAgent"""
        return QueryAnalysisAgentConfig

    def _init_from_config(self, config: QueryAnalysisAgentConfig, component_config: Dict[str, Any],
                          dependencies: Dict[str, Any]) -> None:
        """
        ✅ FRAMEWORK COMPLIANCE: Initialize QueryAnalysisAgent using from_config pattern
        This method is called by the framework's from_config mechanism
        """
        ...
        if hasattr(config, 'confidence_threshold'):
            logger.info(
                f"🔍 DEBUG: config.confidence_threshold value: {config.confidence_threshold}")

        # Agent-specific configuration - properly typed with QueryAnalysisAgentConfig
        # confidence_threshold is inherited from AgentConfig
        self.confidence_threshold = getattr(
            config, 'confidence_threshold', 0.7)
        self.analysis_depth = getattr(
            config, 'analysis_depth', 'comprehensive')
        self.biological_context_focus = getattr(
            config, 'biological_context_focus', True)
        self.domain_specialization = getattr(
            config, 'domain_specialization', {}) or {}
        self.intent_categories = getattr(config, 'intent_categories', []) or []
        self.parameter_types = getattr(config, 'parameter_types', {}) or {}

        logger.info(
            f"🔬 Query Analysis Agent initialized with confidence threshold: {self.confidence_threshold}")

    def __init__(self, config: QueryAnalysisAgentConfig, **kwargs):
        """Initialize Query Analysis Agent with specialized capabilities"""
        super().__init__(config, **kwargs)

        # This __init__ method is for direct instantiation (not used by framework)
        # The framework uses _init_from_config instead
        self.confidence_threshold = getattr(
            config, 'confidence_threshold', 0.7)
        self.analysis_depth = getattr(
            config, 'analysis_depth', 'comprehensive')
    ...
    async def _process_specialized_request(self,
        ...
        try:
            ...
            analysis_result = self._apply_confidence_scoring(
                analysis_result, query)

            logger.debug(
                f"🔬 Query analysis completed: {analysis_result.get('intent', 'unknown')}")
    ...
    def _parse_analysis_response(self, response: str, analysis_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                "⚠️ Failed to parse JSON response, using text analysis")
