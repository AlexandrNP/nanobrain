...
- Inherits from ConversationalSpecializedAgent for proper agent functionality
...
class VirusExtractionAgent(ConversationalSpecializedAgent):
    """
    ...
    **Advanced Features:**
    ...
    See Also:
        * :class:`ConversationalSpecializedAgent`: Base conversational specialized agent implementation
    ...
    def __init__(self, config=None, **kwargs):
        """Initialize the virus extraction agent.

        CRITICAL FIX: Added missing __init__ method to ensure proper parent initialization
        and LLM client setup. This was the root cause of the LLM client issue.
        """
    ...
    def _initialize_agent_specifics(self, config: AgentConfig, **context) -> None:
        ...
        # Extract virus extraction specific settings from both config.config and root level
        agent_config = getattr(config, 'config', {}) if hasattr(
            config, 'config') else {}

        self.extraction_confidence_threshold = agent_config.get(
            'extraction_confidence_threshold', 0.7)
        self.max_species_candidates = agent_config.get(
            'max_species_candidates', 5)
        self.enable_taxonomy_validation = agent_config.get(
            'enable_taxonomy_validation', True)

        # CRITICAL FIX: Check both config.config and root level for extraction_prompt_template
        self.extraction_prompt_template = (
            agent_config.get('extraction_prompt_template') or
            getattr(config, 'extraction_prompt_template', None)
        )

        # DEBUG: Log configuration loading
        logger.info(f"🔍 DEBUG: Config loading - agent_config keys: {list(agent_config.keys()) if agent_config else 'None'}")
        logger.info(f"🔍 DEBUG: Config loading - config attributes: {[attr for attr in dir(config) if not attr.startswith('_')]}")
        logger.info(f"🔍 DEBUG: Config loading - extraction_prompt_template: {self.extraction_prompt_template is not None}")
        if self.extraction_prompt_template:
            logger.info(f"🔍 DEBUG: Config loading - template preview: {self.extraction_prompt_template[:100]}...")
        ...
        logger.info(
            f"🦠 Virus Extraction Agent initialized with confidence threshold: {self.extraction_confidence_threshold}")
    ...
    async def _process_specialized_request(self, input_text: str, **kwargs) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"❌ Error in virus extraction processing: {e}", exc_info=True)
            return None
    ...
    def _should_handle_virus_extraction(self, input_text: str, **kwargs) -> bool:
        ...
        indicator_count = sum(
            1 for indicator in extraction_indicators if indicator in text_lower)
    ...
    async def _extract_virus_species_llm(self, input_text: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self, 'extraction_prompt_template') and self.extraction_prompt_template:
            ...
            logger.error(
                f"❌ Error in LLM virus extraction: {e}", exc_info=True)
            return {
                'virus_species': None,
                'confidence': 0.0,
...
"""
    ...
    async def _call_llm_with_prompt(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        Uses direct LLM client access to avoid recursion issues.
        FIXED: Use direct LLM client instead of self.process() to prevent infinite recursion.
        """
        try:
            # FIXED: Use direct LLM client access instead of self.process() to avoid recursion
            if not hasattr(self, 'llm_client') or not self.llm_client:
                logger.error("❌ No LLM client available for virus extraction")
                return {'content': '', 'error': 'No LLM client available'}

            # Create messages for LLM call - ensure content is properly formatted as strings
            system_content = "Extract virus names. Return only JSON format: {\"virus_species\": \"name or null\"}"
            user_content = str(prompt)  # Ensure prompt is a string
            ...
            # Debug logging to verify message structure
            logger.debug(f"🔍 LLM messages structure: system_content type={type(system_content)}, user_content type={type(user_content)}")
            logger.debug(f"🔍 User content length: {len(user_content)} characters")

            # Call LLM directly using inherited _call_llm method
            ...
            if llm_response and "choices" in llm_response and llm_response["choices"]:
                ...
                try:
                    ...
                    return {'content': content, 'error': 'Invalid JSON response'}

            return {'content': '', 'error': 'No response from LLM'}
        ...
        try:
            ...
            try:
                ...
                if json_start >= 0 and json_end > json_start:
                    ...
                    # Extract virus species from ultra-simple response
                    virus_species = parsed_data.get('virus_species')

                    return {
                        'virus_species': virus_species,
                        'confidence': 0.9 if virus_species else 0.0,
                        'reasoning': f'Extracted virus: {virus_species}' if virus_species else 'No virus found',
                        'analysis_type': 'pssm' if virus_species else 'conversational',
                        'routing_decision': 'virus_name_resolution' if virus_species else 'conversational_response'
                    }
            ...
            logger.error(
                f"❌ Error parsing LLM extraction response: {e}", exc_info=True)
            return {
                'virus_species': None,
                'confidence': 0.0,
    ...
    async def _validate_taxonomy(self, extraction_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"❌ Error in taxonomy validation: {e}", exc_info=True)
            extraction_result['taxonomy_validation'] = {
                'validated': False,
                'error': str(e)
            }
