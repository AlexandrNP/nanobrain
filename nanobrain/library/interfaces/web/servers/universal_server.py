...
class UniversalNanoBrainServer(BaseUniversalServer):
    ...
    def _init_from_config(self, config, component_config, dependencies):
        ...
        if dependencies:
            ...
            logger.info(
                f"✅ Loaded {len(loaded_components)} components: {loaded_components}")
        else:
            ...
            logger.warning(
                "⚠️ No dependencies provided - components will be None")
    ...
    def configure_server_implementation(self) -> None:
        ...
        try:
            ...
            logger.error(f"❌ FastAPI configuration failed: {e}", exc_info=True)
            raise
        ...
        try:
            ...
            async def get_workflow_capabilities():
                ...
                try:
                    ...
                    logger.error(
                        f"❌ Capabilities endpoint error: {e}", exc_info=True)
                    raise HTTPException(status_code=500, detail=str(e))
            ...
            async def analyze_request_routing(request: ChatRequest):
                ...
                try:
                    if not self.request_analyzer or not self.workflow_registry:
                        raise RuntimeError(
                            "Request analysis components not configured")
            ...
            logger.info(
                f"✅ Configured {len(self.config.endpoints) + 2} endpoints")

        except Exception as e:
            logger.error(
                f"❌ Endpoint configuration failed: {e}", exc_info=True)
            raise
    ...
    def initialize_middleware(self) -> None:
        ...
        try:
            ...
            if self.config.middleware.get('request_validation', True):
                logger.debug(
                    "✅ Request validation middleware enabled (FastAPI)")
            ...
            if self.config.middleware.get('response_standardization', True):
                ...
                logger.debug(
                    "✅ Response standardization middleware configured")

            logger.info("✅ All middleware initialized successfully")

        except Exception as e:
            logger.error(
                f"❌ Middleware initialization failed: {e}", exc_info=True)
            raise
    ...
    async def validate_component_initialization(self) -> None:
        ...
        required_components = [
            'workflow_registry', 'request_analyzer', 'workflow_router', 'response_processor']
    ...
    async def auto_discover_workflows(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"✅ Discovered {len(discovery_result.discovered_workflows)} workflows")
        except Exception as e:
    ...
    async def validate_component_health(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"❌ Component health validation failed: {e}", exc_info=True)
            raise
    ...
    async def preload_workflows(self) -> None:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Workflow preloading failed (non-critical): {e}", exc_info=True)
    ...
    async def start(self, host: Optional[str] = None, port: Optional[int] = None) -> None:
        ...
        logger.info(
            f"🚀 Starting Universal NanoBrain Server on {server_host}:{server_port}")
        ...
        try:
            ...
            # Start server
            logger.info(
                f"✅ Universal NanoBrain Server running on http://{server_host}:{server_port}")
            logger.info(
                f"📚 API Documentation: http://{server_host}:{server_port}/docs")
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            logger.error(f"❌ Server shutdown error: {e}", exc_info=True)
            raise
