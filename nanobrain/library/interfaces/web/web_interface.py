    ...
    **Deployment Scenarios:**
    ...
    def _init_from_config(self, config: WebInterfaceConfig, component_config: Dict[str, Any],
        ...
        if isinstance(config, str):
            ...
            try:
                ...
                self.logger.info(
                    f"✅ Loaded WebInterface configuration from file: {config}")
            except Exception as e:
                # Fallback to default configuration if loading fails
                self.config = WebInterfaceConfig()
                self.logger = get_logger("web_interface", "interfaces")
                self.logger.error(
                    f"Failed to load web interface config: {e}", exc_info=True)
                self.logger.warning(
                    f"⚠️ Failed to load config from {config}, using default: {e}")
        else:
        ...
        self.chat_workflow: Optional[ChatWorkflow] = dependencies.get(
            'chat_workflow')
        self.universal_server = dependencies.get('universal_server')
        self.server = None

        self.logger.info("Web interface created", config_name=getattr(
            self.config, 'name', 'web_interface'))
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to initialize web interface: {e}", exc_info=True)
            raise
        ...
        try:
            ...
            if workflow_config:
                ...
                if isinstance(workflow_config, str):
                    # Path to config file
                    self.chat_workflow = ChatWorkflow.from_config(
                        workflow_config)
                    ...
                    import importlib
                    module_path, class_name = workflow_class_path.rsplit(
                        '.', 1)
                    module = importlib.import_module(module_path)
                    workflow_class = getattr(module, class_name)
                    self.chat_workflow = workflow_class.from_config(
            ...
            self.logger.error(
                f"Failed to initialize chat workflow: {e}", exc_info=True)
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to initialize fallback workflow: {fallback_error}")
                raise
        ...
        if isinstance(api_config, dict):
            ...
            self.app = FastAPI(
                ...
                description=api_config.get(
                    "description", "NanoBrain Web Interface"),
    ...
    async def _setup_middleware(self) -> None:
        ...
        if isinstance(logging_config, dict):
            ...
            enable_request_logging = logging_config.get(
                "enable_request_logging", False)
            enable_response_logging = logging_config.get(
                "enable_response_logging", False)
            log_requests_body = logging_config.get("log_requests_body", False)
            log_responses_body = logging_config.get(
    ...
    async def start_server(self) -> None:
        ...
        if isinstance(server_config, dict):
            ...
            # ✅ FRAMEWORK COMPLIANT: No hardcoded defaults - YAML configuration required
            ...
            if port is None:
                raise ValueError(
                    "Port configuration required in YAML - no hardcoded defaults allowed per framework principles")
        else:
        ...
        try:
            ...
            config = uvicorn.Config(
                ...
                log_level=(self.config.logging.get('log_level', 'INFO') if isinstance(
                    self.config.logging, dict) else self.config.logging.log_level).lower()
            )
