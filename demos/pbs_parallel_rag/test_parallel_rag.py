...
class ParallelRAGTester:
    ...
    async def setup_workflow(self):
        ...
        self.workflow = Workflow.from_config(str(config_path))
    ...
    async def test_parallel_queries(self):
        ...
        # Create a task for each query using the PARSL executor
        async def process_query_with_parsl(query, query_num):
            """Process a single query through the first step with PARSL."""
            try:
                ...
                # This returns the enhanced prompt
                result = await prompt_step.process({'query': query})

                # Extract enhanced prompt and worker ID
                enhanced_prompt = None
                worker_id = None

                if isinstance(result, dict):
                    # Extract enhanced query text
                    enhanced_prompt = result.get('enhanced_query', result.get('result', str(result)))
                    # Extract worker ID
                    worker_id = result.get('_worker_id') or result.get('worker_id')
                else:
        ...
        if successful > 0:
            print(f"\n{'='*80}")
            print("📝 DETAILED QUERY RESULTS (Enhanced Prompts from PARSL)")
            print(f"{'='*80}")

            for resp in successful_responses:
                query_num = resp.get('query_num', '?')
                query = resp.get('query', 'N/A')
                enhanced_prompt = resp.get('enhanced_prompt')
                worker_id = resp.get('worker_id', 'N/A')
                ...
                # Worker ID
                print(f"\n👷 PARSL Worker ID: {worker_id}")

                # Enhanced prompt
                print(f"\n✨ Enhanced Prompt (Generated by PARSL Worker):")
                if enhanced_prompt:
                    # Format the enhanced prompt nicely
                    if isinstance(enhanced_prompt, str):
                        # Wrap long text
                        import textwrap
                        wrapped = textwrap.fill(enhanced_prompt, width=76, initial_indent='   ', subsequent_indent='   ')
                        print(wrapped)
                    else:
                        print(f"   {enhanced_prompt}")
                else:
                    print(f"   ⚠️  No enhanced prompt available")
