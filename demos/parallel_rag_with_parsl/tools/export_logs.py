...
from demos.parallel_rag_with_parsl.tools.analyze_logs import LogAnalyzer, QueryStats
...
class LogExporter:
    ...
    def export_summary_to_text(self, output_file: str):
        ...
        lines.append(f"  Avg Total Time: {stats['avg_total_time']:.3f}s")
        lines.append(f"  Avg Enhancement Time: {stats['avg_enhancement_time']:.3f}s")
        lines.append(f"  Avg Retrieval Time: {stats['avg_retrieval_time']:.3f}s")
        lines.append(f"  Avg Generation Time: {stats['avg_generation_time']:.3f}s")
        lines.append(f"  Min Total Time: {stats['min_total_time']:.3f}s")
        lines.append(f"  Max Total Time: {stats['max_total_time']:.3f}s")
        lines.append("")
        
        lines.append("Quality:")
        lines.append(f"  Avg Relevance Score: {stats['avg_relevance_score']:.2f}")
        lines.append("")
