#!/usr/bin/env python3
"""
Query Classification Tests for Chatbot Viral Integration

Tests the query classification functionality as specified in 
section 6.1 of the testing plan.
...
class TestQueryClassification:
    ...
    async def test_qc_005_annotation_mixed_intent(self, mock_workflow_components):
        """
        Test QC-005: "What is EEEV and create PSSM?" → annotation
    ...
    async def test_edge_case_empty_query(self, mock_workflow_components):
        ...
        assert result["confidence"] < 0.5, f"Empty query should have low confidence, got {result['confidence']}"
        print(f"✅ Empty query handled appropriately: confidence={result['confidence']}")
    ...
    async def test_edge_case_long_query(self, mock_workflow_components):
        ...
        # Should still classify but potentially with lower confidence
        assert "intent" in result, "Long query should still be classified"
    ...
    async def test_edge_case_special_characters(self, mock_workflow_components):
        ...
        assert "intent" in result, "Query with special characters should be classified"
        assert result["confidence"] >= 0.0, "Confidence should be non-negative"
        ...
        for query, expected_intent in test_cases:
            ...
            async for chunk in real_workflow.process_user_message(query, "test_session_real"):
                ...
                if chunk.get('type') == 'classification':
                    ...
                    assert actual_intent == expected_intent, f"Real workflow: Expected {expected_intent}, got {actual_intent}"
                    assert confidence > 0.5, f"Real workflow: Low confidence {confidence}"
                    
                    print(f"✅ Real workflow classification: {query} → {actual_intent} (confidence: {confidence})")
                    break
            else:
                pytest.fail(f"No classification chunk found for query: {query}")
...
if __name__ == "__main__":
    ...
    async def run_all_tests():
        ...
        test_instance = TestQueryClassification()
        
        await test_instance.test_qc_001_conversational_eeev(mock_components)
        await test_instance.test_qc_002_annotation_pssm(mock_components)
        await test_instance.test_qc_003_conversational_proteins(mock_components)
        await test_instance.test_qc_004_annotation_analyze(mock_components)
        await test_instance.test_qc_005_annotation_mixed_intent(mock_components)
        
        await test_instance.test_classification_accuracy_metrics(mock_components)
        
        # Edge case tests
        await test_instance.test_edge_case_empty_query(mock_components)
        await test_instance.test_edge_case_long_query(mock_components)
        await test_instance.test_edge_case_special_characters(mock_components)
