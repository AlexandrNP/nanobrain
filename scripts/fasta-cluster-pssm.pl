...
# Configurable MAFFT threading (from environment or default)
my $mafft_threads = $ENV{MAFFT_THREADS} || 24;
...
close OUT2;
system "mafft --thread $mafft_threads --quiet --reorder $base/$tmp/Leftover_Seqs.aa>Leftover_Seqs.fa";
...
foreach (@clusterF)
{
	...
	$ali_file =~ s/\.fasta//g;
	print STDERR "\n\nAligning $file\n"; 
	open (IN, "mafft --thread $mafft_threads --quiet --reorder $base/$tmp/clusters/$file |");
